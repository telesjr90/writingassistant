...
The published Phase 7 parent sequence (`PHASE7-IMPL-001` through `PHASE7-IMPL-010`) is complete. Writer Assistant Core is the current roadmap frontier. `PHASE8-IMPL-001` is complete through `PHASE8-IMPL-001-T007`. `PHASE8-IMPL-002` is complete through `PHASE8-IMPL-002-T007`. `PHASE8-IMPL-003` is complete through `PHASE8-IMPL-003-T007`. `PHASE8-IMPL-004` is complete through `PHASE8-IMPL-004-T007`. `PHASE8-IMPL-005` is active through `PHASE8-IMPL-005-T006`.

Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, and derived candidate index contract/helpers/safety regressions in `PHASE8-IMPL-004`. The app still does not extract story knowledge at runtime. The app still has no extraction/model route, frontend extraction/review UI, apply-promotion, or memory/canon mutation. Candidate-only JSON persistence exists under `writer_assistant/candidates/*.json` and derived index helpers exist in `backend/story_knowledge/candidate_index.py` through helper functions only; no backend routes or frontend review UI consume them yet. `PHASE8-IMPL-005` is the active parent for tool evaluation and extraction strategy before any extraction implementation. External story/NLP tools remain evaluation candidates only. dramatica-flow is accepted as `reference-only` for analysis-pattern inspiration (`docs/roadmap/decisions/PHASE8-IMPL-005-dramatica-flow-analysis-only-reference-decision.md`); runtime adapter use is REJECT/DEFER. Narrative Context Protocol is accepted as `reference-only` and future approved-context import/export candidate; Subtxt docs are accepted as `reference-only` and future semantic rubric candidate (`docs/roadmap/decisions/PHASE8-IMPL-005-ncp-subtxt-structural-interpretation-strategy-decision.md`); NCP/Subtxt runtime adapter, import/export, and Subtxt analysis implementation are REJECT/DEFER. T006 accepted `docs/roadmap/decisions/PHASE8-IMPL-005-nlp-extraction-adapter-strategy-decision.md`: first extraction strategy is spaCy-first local deterministic/rule-assisted candidate extraction foundation. segram, BookNLP, GLiNER, LangExtract, and Renard are deferred from the first implementation slice; LangExtract remains later/local-only/model-assisted only if privacy/local model policy is approved, and Renard remains deferred pending GPL/license review and relationship-network scope. Scope is narrowed to exactly nine approved tools/references. Broader candidates (CoreNLP/OpenIE/SUTime, AI-Reader-V2, narrative-blueprint, NovelClaw, NotebookLM workflow, llm_finetuning, ai-llm-project-file-structure-template) remain out of scope unless a later owner-approved parent reintroduces them. First extraction implementation parent remains future work after T007 closeout.
...
- Latest completed frontier parent is `PHASE8-IMPL-004 - Writer Assistant Core candidate index contract and derived index helpers`; it is complete through `PHASE8-IMPL-004-T007 - Roadmap/status closeout`. Active frontier parent is `PHASE8-IMPL-005 - Writer Assistant Core tool evaluation and extraction strategy decision`; it is active through `PHASE8-IMPL-005-T006 - NLP/extraction adapter strategy decision`. Prior completed child under active parent: `PHASE8-IMPL-005-T005 - NCP/Subtxt structural interpretation strategy decision`. Active child: `PHASE8-IMPL-005-T007 - Roadmap/status closeout`.
