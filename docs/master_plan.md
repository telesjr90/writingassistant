...
- Latest completed parent is `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`, complete through `PHASE8-IMPL-009-T007 - Roadmap/status closeout`. Active frontier is pending next parent publication after completed `PHASE8-IMPL-010 - Writer Assistant Core extraction orchestration planning and review-safe pipeline boundary`, complete through `PHASE8-IMPL-010-T007`. The fixture parser has TSV, `.book` JSON, token-event derivation, and raw artifact bundle builder support; the full parser contract passes. The minimal orchestrator helper is pure in-memory and the orchestrator contract passes. T006 validates the review-safe fixture orchestrator as pure, in-memory, fixture-only, candidate-draft-only, non-persistent, non-canon, non-prose, and non-runtime-extraction; candidate persistence, review UI/API, owner decisions, apply-promotion, raw persistence, and runtime extraction remain deferred. The recommended next parent is `PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning`. No raw extraction artifact files exist, no raw file write/read/list helpers exist, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists.
