...
Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, derived candidate index contract/helpers/safety regressions in `PHASE8-IMPL-004`, tool evaluation/extraction strategy in `PHASE8-IMPL-005`, the evidence-first extraction foundation plus BookNLP-ready adapter strategy in `PHASE8-IMPL-006`, the mocked BookNLP adapter contract implementation and normalization foundation in `PHASE8-IMPL-007`, and the raw extraction artifact storage and BookNLP fixture parser contract parent in `PHASE8-IMPL-008`. `PHASE8-IMPL-008-T001` through `PHASE8-IMPL-008-T007` are complete: T002 accepted the project-local raw extraction artifact storage contract, T003 added expected-red storage contract tests, T004 created pure storage path and manifest validation helpers, T005 accepted the in-memory BookNLP fixture parser contract decision, and T006 added `tests/test_writer_assistant_core_booknlp_fixture_parser_contract.py` as expected-red parser contract coverage. `PHASE8-IMPL-009` is active as the BookNLP fixture parser helper implementation and raw artifact bundle integration parent. `PHASE8-IMPL-009-T001` is complete as docs/status/planning publication. `PHASE8-IMPL-009-T002` is complete as the parser implementation contract reconciliation decision and accepted the public parser APIs plus T003-T006 split. `PHASE8-IMPL-009-T003` is complete as the first runtime parser implementation child: it created `backend/story_knowledge/booknlp_fixture_parser.py` and implemented pure in-memory TSV parsers for tokens, entities, quotes, and supersense only. `PHASE8-IMPL-009-T004` is complete as the second runtime parser implementation child: it implemented pure in-memory `.book` JSON parsing and token-event derivation from token `event` markers. `PHASE8-IMPL-009-T005` is complete as the raw artifact bundle builder integration child: it implemented pure in-memory fixture bundle construction with storage/source/evidence validation and adapter bundle compatibility. `PHASE8-IMPL-009-T006` is complete as parser validation and boundary hardening: the full parser contract, source-level boundary scan, and focused regressions passed, with no parser code changes needed. Parser contract collection passes and the full parser contract passes. `PHASE8-IMPL-009-T007` is ready/active as roadmap/status closeout. No raw artifact write/read/list helpers, runtime extraction, real BookNLP/spaCy install or execution, routes, UI, package changes, generated prose, apply-promotion, or memory/canon mutation exists.
...
- Active frontier parent is `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`; `PHASE8-IMPL-009-T001` through `PHASE8-IMPL-009-T006` are complete, and `PHASE8-IMPL-009-T007 - Roadmap/status closeout` is ready/active. Latest completed parent remains `PHASE8-IMPL-008 - Writer Assistant Core raw extraction artifact storage and BookNLP fixture parser contract`, complete through `PHASE8-IMPL-008-T007 - Roadmap/status closeout`. The fixture parser has TSV, `.book` JSON, token-event derivation, and raw artifact bundle builder support; the full parser contract passes. No raw extraction artifact files exist, no raw file write/read/list helpers exist, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists.
