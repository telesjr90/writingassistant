...
The MVP goal is a usable local app that can create/load a project, edit and save owner-authored scenes/notes/materials, run analysis over owner-authored or owner-provided project text, persist raw extraction artifacts safely, create evidence-backed candidates, expose an owner review surface, execute safe owner-review actions, apply explicit owner-approved promotion, and update approved project memory/canon only after confirmation. MVP runtime scope includes real BookNLP/spaCy install/run/import, runtime extraction, raw artifact persistence, read-only and command review surfaces, frontend owner-action execution, apply-promotion, approved memory/canon mutation, model-assisted evidence-backed extraction, and analysis-only NCP/Subtxt/dramatica-flow integration. Fine-tuning is the only major listed feature deferred after MVP; the MVP does not require a fine-tuned model.
...
- `PHASE8-UX-002-T003` - Expected-red tests for source/Story Check/no-prose UI. Complete/PASS as tests-first expected-red only using the repo virtualenv pytest interpreter.
...
- `PHASE8-UX-002-T006` - Implement Notes/Materials + runtime/review evidence UI. In progress; T006A Notes/Materials evidence UI and T006B raw artifact / analysis runtime status UI are complete/PASS, and T006C is next.
...
`WORKSPACE-001` through `WORKSPACE-025` define the Project Workspace Foundation planning specs. `WORKSPACE-026`, documented in `docs/roadmap/project_workspace_implementation_decision_sweep.md`, consolidates those specs into implementation-readiness decisions. Phase 7 implementation is complete through `PHASE7-IMPL-010 - Workspace Validation / Browser and Manual Smoke`: projects can be selected, scenes/notes/materials have storage and routes, the frontend has shared owner-authored editor handling, the workspace includes a deterministic project overview view, OMI-guided project creation has a frontend-transient owner-controlled staged shell, the workspace includes a read-only Memory / Canon shell with approved-only empty states and App/Nav integration, and workspace validation passed automated regression with PARTIAL browser/manual smoke due to environment tooling limits (no confirmed product blocker; no runtime repair required). These paths remain owner-authored or owner-provided content only; no generated prose, summaries, extraction, Story Check auto-runs, model calls, OMI promotion, apply-promotion, memory/canon mutation, backend approved-memory routes/helpers, frontend approved-memory API helpers, hidden pre-confirmation project writes, staged backend storage/routes/API helpers, or training/dataset changes were added.

The published Phase 7 parent sequence (`PHASE7-IMPL-001` through `PHASE7-IMPL-010`) is complete. Writer Assistant Core is the current roadmap frontier. `PHASE8-IMPL-001` is complete through `PHASE8-IMPL-001-T007`. `PHASE8-IMPL-002` is complete through `PHASE8-IMPL-002-T007`. `PHASE8-IMPL-003` is complete through `PHASE8-IMPL-003-T007`. `PHASE8-IMPL-004` is complete through `PHASE8-IMPL-004-T007`. `PHASE8-IMPL-005` is complete through `PHASE8-IMPL-005-T007`.

Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, derived candidate index contract/helpers/safety regressions in `PHASE8-IMPL-004`, tool evaluation/extraction strategy in `PHASE8-IMPL-005`, the evidence-first extraction foundation plus BookNLP-ready adapter strategy in `PHASE8-IMPL-006`, the mocked BookNLP adapter contract implementation and normalization foundation in `PHASE8-IMPL-007`, the raw extraction artifact storage and BookNLP fixture parser contract parent in `PHASE8-IMPL-008`, and the BookNLP fixture parser helper implementation and raw artifact bundle integration parent in `PHASE8-IMPL-009`. `PHASE8-IMPL-009-T001` through `PHASE8-IMPL-009-T007` are complete: T002 accepted the public parser APIs plus T003-T006 split, T003 created `backend/story_knowledge/booknlp_fixture_parser.py` and implemented pure in-memory TSV parsers, T004 implemented pure in-memory `.book` JSON parsing and token-event derivation, T005 implemented pure in-memory fixture bundle construction with storage/source/evidence validation and adapter bundle compatibility, T006 validated the full parser contract and focused regressions with no parser code changes needed, and T007 closed the parent. Parser contract collection passes and the full parser contract passes. `PHASE8-IMPL-010 - Writer Assistant Core extraction orchestration planning and review-safe pipeline boundary` is complete through `PHASE8-IMPL-010-T007`. It is review-safe extraction orchestration planning, not real extraction. `PHASE8-IMPL-010-T003` added expected-red extraction orchestration contract tests in `tests/test_writer_assistant_core_extraction_orchestrator_contract.py`, `PHASE8-IMPL-010-T004` completed the minimal pure helper in `backend/story_knowledge/extraction_orchestrator.py` with `validate_extraction_pipeline_request`, `build_fixture_extraction_pipeline_plan`, and `run_fixture_extraction_pipeline`, and `PHASE8-IMPL-010-T005` completed the candidate review handoff and persistence boundary decision. `PHASE8-IMPL-010-T006` completed a validation-only orchestration safety regression pass with no runtime hardening patch needed, and `PHASE8-IMPL-010-T007` closed the parent. The orchestrator contract passes with 67 tests, and T006 validated parser, raw storage, adapter, source/evidence, candidate, and focused OMI/project regressions. `PHASE8-IMPL-010` is complete. No raw artifact write/read/list helpers, runtime extraction, real BookNLP/spaCy install or execution, routes, UI, package changes, generated prose, apply-promotion, or memory/canon mutation exists.

`PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning` is complete through `PHASE8-IMPL-011-T007`. `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning` is complete through `PHASE8-IMPL-012-T007` and delivered safe queue workflow infrastructure: review queue storage and owner action record validation only, a project-local queue storage helper `backend/story_knowledge/review_queue_storage.py`, and a derived/rebuildable index. `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` is complete through `PHASE8-IMPL-013-T007`. `PHASE8-IMPL-014 - Writer Assistant Core review API implementation and tests-first route boundary` is complete through `PHASE8-IMPL-014-T007`. `PHASE8-IMPL-015 - Review API route implementation and read-only frontend review queue surface` is complete through `PHASE8-IMPL-015-T007`. `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary` is complete through `PHASE8-IMPL-016-T007`. `PHASE8-IMPL-017 - Apply-promotion contract, audit log, and approved memory/canon mutation boundary` is complete through `PHASE8-IMPL-017-T007`; apply-promotion is explicit owner-confirmed only, audited, evidence/provenance/source-locator-backed, fail closed, and the only approved path from candidate/review workflow state into approved memory/canon for that parent. `PHASE8-IMPL-018 - Raw artifact persistence implementation and project-local extraction artifact lifecycle` is complete/PASS through `PHASE8-IMPL-018-T007`. `PHASE8-IMPL-019 - Guarded runtime extraction: real BookNLP/spaCy install/run/import and candidate-first extraction pipeline` is complete/PASS through `PHASE8-IMPL-019-T007`. `PHASE8-IMPL-020 - Model-assisted evidence-backed extraction and diagnostic workflow` is complete/PASS through `PHASE8-IMPL-020-T007`. `PHASE8-IMPL-021 - Analysis-only NCP/Subtxt/dramatica-flow runtime integration` is complete/PASS through `PHASE8-IMPL-021-T007`. `PHASE8-IMPL-022 - End-to-end MVP usability validation` is complete/PASS through `PHASE8-IMPL-022-T007`; T007 is docs/status/governance closeout only. Final artifacts are `docs/roadmap/decisions/PHASE8-IMPL-022-mvp-usability-validation-matrix-decision.md`, `backend/story_knowledge/mvp_usability_smoke.py`, `tests/test_writer_assistant_core_mvp_usability_smoke_contract.py`, `tests/test_writer_assistant_core_mvp_usability_smoke_workflow_contract.py`, and `tests/test_writer_assistant_core_mvp_usability_smoke_safety_regression.py`. PHASE8-IMPL-022 validates the complete MVP path through a deterministic in-memory smoke harness while preserving candidate-first, owner review required, evidence/provenance/source-locator-backed behavior when available, confidence is not truth, tool output is not canon, model output is not canon, no automatic canon, no apply-promotion outside explicit audited owner-confirmed path, no memory/canon mutation outside owner-approved workflow, no training artifacts, no generated prose, no rewrite, no continuation, no outline, fail closed, no silent fallback, queue presence is not approval, and candidate persistence is not canon. PHASE8-IMPL-022 added no routes, frontend code, package/dependency changes, real runtime extraction execution, BookNLP/spaCy execution, NCP/Subtxt/dramatica-flow execution, model/Ollama calls, network/subprocess behavior, runtime project file writes, candidate persistence writes, review queue writes, apply-promotion behavior changes, approved memory/canon mutation, training/JSONL/dataset/model artifacts, generated prose, or prose-production behavior. No active child remains for PHASE8-IMPL-022. `PHASE8-UX-002 - MVP acceptance UI completion and route wiring` is now the active MVP-first UX parent after PHASE8-IMPL-022. `PHASE8-UX-002-T001` is complete/PASS as docs/status/planning parent publication only, `PHASE8-UX-002-T002` is complete/PASS as docs/decision/planning only, and `PHASE8-UX-002-T003` is the next planned expected-red test child. This does not declare the whole MVP complete or record owner acceptance. Fine-tuning remains deferred after MVP. Generated prose/prose-production paths remain permanently forbidden and are not future roadmap features.

`PHASE8-IMPL-021` is complete/PASS as the analysis-only NCP/Subtxt/dramatica-flow runtime integration parent. `PHASE8-IMPL-021-T001` through `PHASE8-IMPL-021-T007` are complete/PASS. Final artifacts are `docs/roadmap/decisions/PHASE8-IMPL-021-analysis-only-runtime-integration-boundary-allowlist-decision.md`, `backend/story_knowledge/analysis_runtime_integration.py`, `tests/test_writer_assistant_core_analysis_runtime_integration_contract.py`, `tests/test_writer_assistant_core_analysis_runtime_integration_handoff_contract.py`, and `tests/test_writer_assistant_core_analysis_runtime_integration_safety_regression.py`. Final behavior includes the analysis-only NCP/Subtxt/dramatica-flow runtime boundary and audited allowlist decision, expected-red analysis runtime contract tests, pure in-memory analysis runtime guard helper, allowlist validation, request validation, non-executing plan building, output validation, candidate support shaping, diagnostic question shaping, quarantine/fail-closed wrapping, guarded runtime state reporting, evidence-backed candidate/diagnostic/review handoff objects, handoff validation, safety regression coverage, and minimal hardening for unsafe refs/allowlist values, missing-ref status preservation, nested forbidden output rejection, prose/improvement intent refusal, and explicit non-execution flags. NCP is structured context interchange only. Subtxt is rubric/diagnostic guidance only. dramatica-flow is audited allowlist only. Runtime outputs are candidate-first and owner review is required; `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs` are preserved when available. Confidence is not truth; tool output is not canon; tool output is not truth; no model output as truth; no automatic canon; no apply-promotion; no memory/canon mutation; no training artifacts; no generated prose; no rewrite; no continuation; no outline; fail closed; no silent fallback; queue presence is not approval; candidate persistence is not canon; apply-promotion remains the separate explicit audited owner-confirmed path. PHASE8-IMPL-021 added no NCP execution, no Subtxt execution, no dramatica-flow execution, no repository cloning, no dependency installation, no model/Ollama calls, no network/subprocess runtime behavior, no routes, no UI, no frontend changes, no package/dependency changes, no persistence, no candidate persistence, no review queue entries, no approved memory/canon mutation, no apply-promotion, no training/JSONL/dataset/model artifacts, and no generated prose, rewrite, continuation, outline, draft, revision, polish, expansion, style imitation, export-as-prose, chapter prose, or story prose. `PHASE8-IMPL-022` is active as the MVP-required end-to-end usability validation parent.

`PHASE8-IMPL-019 - Guarded runtime extraction: real BookNLP/spaCy install/run/import and candidate-first extraction pipeline` is complete/PASS through `PHASE8-IMPL-019-T007`. `PHASE8-IMPL-019-T001` through `PHASE8-IMPL-019-T007` are complete/PASS. The final parent result preserves explicit runtime extraction environment/availability states, BookNLP/spaCy availability/probe guard shape, path-safe request validation, owner-authored/owner-provided source boundary, `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs`, deterministic plan building, transient probe result behavior, guarded fail-closed runtime execution shell, PHASE8-IMPL-018 raw artifact handoff persistence, candidate-review draft handoff only, quarantine handling, no silent fallback, and no full BookNLP/spaCy extraction over project text yet. Final artifacts are `backend/story_knowledge/runtime_extraction.py`, `tests/test_writer_assistant_core_runtime_extraction_contract.py`, `tests/test_writer_assistant_core_runtime_extraction_raw_artifact_handoff_contract.py`, `tests/test_writer_assistant_core_runtime_extraction_safety_regression.py`, and `docs/roadmap/decisions/PHASE8-IMPL-019-guarded-runtime-extraction-boundary-environment-model-decision.md`. `PHASE8-IMPL-020 - Model-assisted evidence-backed extraction and diagnostic workflow` is complete/PASS through `PHASE8-IMPL-020-T007`. `PHASE8-IMPL-020-T001` is complete/PASS; `PHASE8-IMPL-020-T002` is complete/PASS as docs/decision only; `PHASE8-IMPL-020-T003` is complete/PASS as expected-red tests-only handoff; `PHASE8-IMPL-020-T004` is complete/PASS as pure request/output guard implementation at `backend/story_knowledge/model_assisted_extraction.py`; `PHASE8-IMPL-020-T005` is complete/PASS as pure in-memory diagnostic/candidate/review handoff implementation; `PHASE8-IMPL-020-T006` is complete/PASS as focused safety regression with minimal helper hardening; and `PHASE8-IMPL-020-T007` is complete/PASS as docs/status/governance closeout only. Final artifacts are `docs/roadmap/decisions/PHASE8-IMPL-020-model-assisted-extraction-diagnostic-workflow-boundary-decision.md`, `backend/story_knowledge/model_assisted_extraction.py`, `tests/test_writer_assistant_core_model_assisted_extraction_contract.py`, `tests/test_writer_assistant_core_model_assisted_extraction_handoff_contract.py`, and `tests/test_writer_assistant_core_model_assisted_extraction_safety_regression.py`. Final behavior includes the model-assisted extraction boundary decision, pure model-assisted extraction guard helper, request validation, environment validation, in-memory prompt packet shaping, output validation, in-memory candidate support shaping, in-memory diagnostic question shaping, quarantine/fail-closed handling, in-memory diagnostic/candidate/review handoff objects, safety regression coverage, and minimal validation hardening for forbidden output classes, nested forbidden outputs, source locator refs, path traversal variants, and blocked forced success states. Model-assisted output may produce evidence-backed candidate observations, diagnostic questions, uncertainty notes, insufficient-evidence notes, safe refusal results, quarantined/fail-closed results, or candidate extraction support only; `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs` are required when available. Confidence is not truth; model output is not canon; no model output as truth; no automatic canon; no apply-promotion; no memory/canon mutation; no training artifacts; no generated prose; no rewrite; no continuation; no outline; fail closed; and no silent fallback remain required boundaries. Queue presence is not approval. Candidate persistence is not canon. PHASE8-IMPL-020 added no model/Ollama calls, no `backend.analysis_engine` imports/calls, no routes, no UI, no frontend changes, no package/dependency changes, no BookNLP/spaCy extraction, no NCP/Subtxt/dramatica-flow runtime, no candidate persistence, no review queue entry creation, no apply-promotion changes, no approved memory/canon mutation, no bible/storyform/scenes/notes/materials mutation, no training/JSONL/dataset/model artifacts, and no generated prose, rewrite, continuation, outline, draft, revision, polish, expansion, style imitation, or story prose. PHASE8-UX-001 remains read-only terminology/boundary reference only and does not override roadmap truth. `PHASE8-IMPL-021` is the recommended next MVP-required parent after review. `PHASE8-IMPL-022` remains future MVP-required; fine-tuning remains deferred after MVP; generated prose/prose-production paths remain permanently forbidden.

`PHASE8-IMPL-010` is complete as the prior Writer Assistant Core parent. `PHASE8-IMPL-010-T001` published the parent task record, inventory, enrichment JSON, and roadmap/status updates. `PHASE8-IMPL-010-T002` is complete as of 2026-06-23 with the review-safe extraction pipeline contract decision at `docs/roadmap/decisions/PHASE8-IMPL-010-review-safe-extraction-pipeline-contract-decision.md`; T002 accepted the review-safe pipeline shape, synthetic-fixture-only first orchestration mode, future pure orchestrator module name `backend/story_knowledge/extraction_orchestrator.py` with public APIs `validate_extraction_pipeline_request`, `build_fixture_extraction_pipeline_plan`, and `run_fixture_extraction_pipeline`, strict safe policy flags, in-memory candidate drafts only, no persistence in PHASE8-IMPL-010, owner-review-first handoff, and fail-closed/quarantine behavior. `PHASE8-IMPL-010-T003` is complete as of 2026-06-24 with expected-red contract tests. `PHASE8-IMPL-010-T004` is complete as of 2026-06-24 with the minimal review-safe orchestration helper at `backend/story_knowledge/extraction_orchestrator.py`; the orchestrator contract passes with 67 tests. `PHASE8-IMPL-010-T005` is complete as of 2026-06-24 with the candidate review handoff and persistence boundary decision at `docs/roadmap/decisions/PHASE8-IMPL-010-candidate-review-handoff-persistence-boundary-decision.md`. This parent decides how source maps, evidence records, raw extraction storage helpers, the in-memory BookNLP fixture parser, raw artifact bundle validation, BookNLP adapter normalization/draft support, candidate persistence/index helpers, and owner review fit together before any real runtime extraction. Candidate drafts remain in-memory review support only; candidate persistence, review UI/API, owner decisions, apply-promotion, raw persistence, and runtime extraction remain deferred. `PHASE8-IMPL-010-T006` is complete as of 2026-06-24 as a validation-only safety regression pass; `PHASE8-IMPL-010-T007` is complete as of 2026-06-24 and closes the parent. The recommended next parent is `PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning`, recommendation-only until separately published. No real BookNLP/spaCy install/run/import is authorized. No runtime extraction exists yet. No prose generation, rewrite, continuation, automatic Storyform truth, apply-promotion, or memory/canon mutation is allowed.
...
This foundation comes before fine-tuning, graph/timeline visualization, relationship-network automation, RunPod work, or Books 4-5 promotion. Runtime extraction, raw artifact persistence, real BookNLP/spaCy runtime, model-assisted extraction, analysis-only NCP/Subtxt/dramatica-flow integration, frontend owner-action execution, apply-promotion, and approved memory/canon mutation are no longer after-MVP extras; they are required before MVP usability/testing is complete.
...
- No story-prose-production controls.
...
- Use `docs/roadmap/omi_ideas_candidates_page_spec.md` as the WORKSPACE-012 planning handoff for the future dedicated OMI raw idea, candidate review, evidence/provenance, promotion readiness, promotion audit record, local filter/search, and warning-state page.
- Use `docs/roadmap/approved_characters_page_spec.md` as the WORKSPACE-013 planning handoff for the first approved-only category page: applied `character_memory_record` display, evidence/provenance panel, linked sources panel, candidate backlog snapshot, warning states, API/UI planning, no-prose/no-silent-promotion boundaries, and future tests.
- Use `docs/roadmap/approved_locations_settings_page_spec.md` as the WORKSPACE-014 planning handoff for the second approved-only category page: applied `location_memory_record` display, evidence/provenance panel, linked sources panel, place hierarchy placeholder, scene usage snapshot, candidate backlog snapshot, warning states including parent/child cycle detection, API/UI planning, no-prose/no-silent-promotion boundaries, and future tests.
- Use `docs/roadmap/approved_timeline_page_spec.md` as the WORKSPACE-015 planning handoff for the third approved-only category page: applied `timeline_event_memory_record` display, evidence/provenance panel, linked sources panel, chronology/ordering placeholder, cause/effect placeholder, scene usage snapshot, candidate backlog snapshot, warning states including sequence collisions, chronology conflicts, and cause/effect cycles, API/UI planning, no-prose/no-silent-promotion boundaries, and future tests.
- Use `docs/roadmap/approved_plot_threads_page_spec.md` as the WORKSPACE-016 planning handoff for the fourth approved-only category page: applied `plot_thread_memory_record` display, evidence/provenance panel, linked sources panel, linked timeline/scene snapshot, related characters/locations/objects snapshot, related open-questions and continuity-warnings placeholders, candidate backlog snapshot, warning states including thread status conflicts, API/UI planning, no-prose/no-silent-promotion boundaries, and future tests.
- Use `docs/roadmap/continuity_consistency_page_spec.md` as the WORKSPACE-017 planning handoff for the Continuity / Consistency page: applied `continuity_warning_memory_record` or `consistency_issue_memory_record` display, evidence/provenance panel, linked sources panel, affected story-knowledge snapshot, resolution placeholder, candidate backlog snapshot, warning states including status/resolution conflicts, API/UI planning, no-prose/no-silent-promotion boundaries, and future tests.
- Use `docs/roadmap/approved_open_questions_page_spec.md` as the WORKSPACE-018 planning handoff for the Approved Open Questions page: applied `open_question_memory_record` or `unresolved_question_memory_record` display, evidence/provenance panel, linked sources panel, linked story-knowledge snapshot, owner-answer/resolution placeholder, candidate backlog snapshot, warning states including answer/resolution conflicts and broken/cyclic related-question links, API/UI planning, no-prose/no-silent-promotion boundaries, and future tests.
- Use `docs/roadmap/approved_relationships_page_spec.md` as the WORKSPACE-019 planning handoff for the Approved Relationships page: applied `relationship_memory_record` display, participant snapshot, linked story-knowledge snapshot, related plot/timeline/open-question/continuity snapshots, evidence/provenance panel, candidate backlog snapshot, warning states including participant/type/status conflicts and broken related links, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica Relationship Story boundaries.
- Use `docs/roadmap/approved_organizations_groups_page_spec.md` as the WORKSPACE-020 planning handoff for the Approved Organizations / Groups page: applied `organization_memory_record` or `group_memory_record` display, members/leadership snapshot, hierarchy snapshot, linked story-knowledge snapshot, related relationship/plot/timeline/open-question/continuity snapshots, evidence/provenance panel, candidate backlog snapshot, warning states including membership/leadership conflicts, hierarchy cycles, type/status conflicts, and broken related links, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica structural-claim boundaries.
- Use `docs/roadmap/approved_objects_items_page_spec.md` as the WORKSPACE-021 planning handoff for the Approved Objects / Items page: applied `object_memory_record` or `item_memory_record` display, ownership/holder snapshot, location/movement snapshot, linked story-knowledge snapshot, related plot/timeline/open-question/continuity snapshots, evidence/provenance panel, candidate backlog snapshot, warning states including ownership/holder conflicts, location/movement conflicts, type/status conflicts, broken related links, and explicit non-Dramatica structural/thematic-claim boundaries.
- Use `docs/roadmap/approved_annotations_evidence_provenance_page_spec.md` as the WORKSPACE-022 planning handoff for the Approved Annotations / Evidence / Provenance page: approved annotation/evidence/provenance record display, source locator panel, evidence span panel, provenance chain panel, linked approved-memory snapshot, linked candidate/promotion audit snapshot, copyright/source safety notice, candidate backlog snapshot, warning states including broken locators, unsafe excerpts, hash mismatches, and broken/cyclic provenance chains, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica proof boundaries.
- Use `docs/roadmap/approved_contradictions_page_spec.md` as the WORKSPACE-023 planning handoff for the Approved Contradictions page: approved contradiction/cross-record conflict record display, claim pair panel, evidence/provenance panel, linked sources panel, linked approved-memory/story-knowledge snapshots, related continuity/open-question snapshots, resolution placeholder, candidate backlog snapshot, warning states including broken claim source links, locator warnings, severity/status/resolution conflicts, unsafe excerpts, and explicit non-Dramatica proof boundaries.
...
- Latest completed parent is `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary`, complete through `PHASE8-IMPL-016-T007 - Roadmap/status closeout`. Current active parent is `PHASE8-IMPL-017 - Apply-promotion contract, audit log, and approved memory/canon mutation boundary`; ready/active next pending T001 publication. `PHASE8-IMPL-016` delivered the backend review-action command route/helper, frontend owner-action review workflow/surface, and focused safety regression coverage. It did not deliver apply-promotion, approved memory/canon mutation, raw artifact persistence, runtime extraction, model-assisted extraction, real BookNLP/spaCy install/run/import, NCP/Subtxt/dramatica-flow runtime integration, or generated prose/prose-production paths. Future MVP-required parents remain `PHASE8-IMPL-017` through `PHASE8-IMPL-022`. Fine-tuning remains deferred after MVP. Generated prose/prose-production paths remain permanently forbidden. Generated context artifacts remain evidence only and must not be treated as roadmap truth.
...
- Initial project-specific pages for chapters/scenes, notes/materials, OMI ideas/candidates, approved characters, locations/settings, objects/items, timeline, plot threads, continuity/consistency, open questions, relationships, organizations/groups, annotations/evidence/provenance, and approved canon/memory.
...
WORKSPACE-017 planning handoff: `docs/roadmap/continuity_consistency_page_spec.md` defines the future Continuity / Consistency page in implementation-ready planning detail: approved-only continuity/consistency records, candidate separation, evidence/provenance, linked sources, linked story-knowledge snapshots, resolution placeholders, warning states, API/UI planning, and no-prose/no-silent-promotion boundaries. It is documentation only and does not implement runtime behavior.

WORKSPACE-018 planning handoff: `docs/roadmap/approved_open_questions_page_spec.md` defines the future Approved Open Questions page in implementation-ready planning detail: approved-only open-question records, candidate separation, evidence/provenance, linked sources, linked story-knowledge snapshots, owner-answer/resolution placeholders, warning states, API/UI planning, and no-prose/no-silent-promotion boundaries. It is documentation only and does not implement runtime behavior.

WORKSPACE-019 planning handoff: `docs/roadmap/approved_relationships_page_spec.md` defines the future Approved Relationships page in implementation-ready planning detail: approved-only relationship records, candidate separation, participant snapshots, evidence/provenance, linked sources, linked story-knowledge snapshots, related plot/timeline/open-question/continuity snapshots, warning states, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica Relationship Story boundaries. It is documentation only and does not implement runtime behavior.

WORKSPACE-020 planning handoff: `docs/roadmap/approved_organizations_groups_page_spec.md` defines the future Approved Organizations / Groups page in implementation-ready planning detail: approved-only organization/group records, candidate separation, members/leadership and hierarchy snapshots, evidence/provenance, linked sources, linked story-knowledge snapshots, related relationship/plot/timeline/open-question/continuity snapshots, warning states, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica structural-claim boundaries. It is documentation only and does not implement runtime behavior.
WORKSPACE-021 planning handoff: `docs/roadmap/approved_objects_items_page_spec.md` defines the future Approved Objects / Items page in implementation-ready planning detail: approved-only object/item records, candidate separation, ownership/holder and location/movement snapshots, evidence/provenance, linked sources, linked story-knowledge snapshots, related plot/timeline/open-question/continuity snapshots, warning states, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica structural/thematic-claim boundaries. It is documentation only and does not implement runtime behavior.
WORKSPACE-022 planning handoff: `docs/roadmap/approved_annotations_evidence_provenance_page_spec.md` defines the future Approved Annotations / Evidence / Provenance page in implementation-ready planning detail: approved-only annotation/evidence/provenance records, candidate separation, source locator panel, evidence span panel, provenance chain panel, linked approved-memory snapshot, linked candidate/promotion audit snapshot, related story-knowledge snapshot, copyright/source safety notice, warning states, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica proof boundaries. It is documentation only and does not implement runtime behavior.
WORKSPACE-023 planning handoff: `docs/roadmap/approved_contradictions_page_spec.md` defines the future Approved Contradictions page in implementation-ready planning detail: approved-only contradiction/cross-record conflict records, candidate separation, claim pair panel, evidence/provenance panel, linked sources, linked approved-memory and story-knowledge snapshots, related continuity/open-question snapshots, resolution placeholder, warning states, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica proof boundaries. It is documentation only and does not implement runtime behavior.
WORKSPACE-024 planning handoff: `docs/roadmap/approved_scene_event_causality_review_spec.md` defines the future Approved Scene / Event / Causality Review page in implementation-ready planning detail: approved-only scene-review, event/action, and causality-note records; candidate separation; source locator, cause/effect, evidence/provenance, linked sources, linked approved-memory, related timeline/plot/continuity/contradiction, and candidate backlog panels; warning states; API/UI planning; no-prose/no-silent-promotion boundaries; and explicit non-Dramatica proof boundaries. It is documentation only and does not implement runtime behavior.
WORKSPACE-025 planning handoff: `docs/roadmap/project_memory_canon_cross_linking_health_spec.md` defines future project-level approved memory/canon cross-linking and health in implementation-ready planning detail: shared record/link identity, `memory/index.json` and category registry planning, category count snapshots, reference/index/ID/schema/source/evidence warning classes, local health filtering, API/UI planning, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica proof boundaries. It is documentation only and does not implement runtime behavior.
