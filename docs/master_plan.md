...
- Latest completed parent is `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract`, complete through `PHASE8-IMPL-013-T007 - Roadmap/status closeout`. The current active parent is `PHASE8-IMPL-014 - Writer Assistant Core review API implementation and tests-first route boundary`, active after `PHASE8-IMPL-014-T001 - Publish review API implementation and tests-first route boundary parent` (docs/status/planning only). `PHASE8-IMPL-013` delivered read-only review queue API contract, owner action command API contract, review UI planning boundary, and expected-red `backend.review_api` tests at `tests/test_writer_assistant_review_api_contract.py`. `PHASE8-IMPL-014-T001` published the parent task record, inventory, enrichment JSON, and roadmap/status updates only; T001 is complete, T002 is ready/active as docs/decision only, and T003-T007 are planned. T001 did not implement `backend.review_api`, did not register FastAPI routes, did not implement frontend review UI or frontend API helpers, did not implement owner action execution, did not implement apply-promotion, did not mutate memory/canon, did not persist raw artifacts, did not implement runtime extraction, did not install or run BookNLP/spaCy, and did not change package/dependency files, tests, training/JSONL/dataset files, or project runtime files. No route/API/UI implementation, owner action execution, apply-promotion, memory/canon mutation, runtime extraction, raw artifact persistence, generated prose, package changes, model calls, raw artifact writes, or training/JSONL/dataset work is implemented or claimed.
