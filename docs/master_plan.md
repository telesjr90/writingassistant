...
`PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning` is complete through `PHASE8-IMPL-011-T007`. `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning` is complete through `PHASE8-IMPL-012-T007` (docs/status closeout only). `PHASE8-IMPL-012` was a planning/contract parent that decided whether and how the in-memory review queue entries produced by the completed `PHASE8-IMPL-011` candidate review gate should be stored, listed, loaded, and prepared for future owner-review workflows without turning queue state into approval, canon truth, apply-promotion, review UI/API scope creep, or memory/canon mutation. It is implemented and tested as safe queue workflow infrastructure: review queue storage and owner action record validation only, a project-local queue storage helper `backend/story_knowledge/review_queue_storage.py`, and a derived/rebuildable index, with the target review queue storage contract passing (359 tests) and candidate review gate/candidate/orchestrator/source-evidence/parser-storage-adapter/OMI regressions passing. No review UI/API, backend routes, frontend UI, apply-promotion, memory/canon mutation, raw artifact persistence, or runtime extraction exists. `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` is now published and active after `PHASE8-IMPL-013-T002` (docs/decision only); it is a planning/contract parent that decides whether and how the project-local review queue storage and owner action record validation should be exposed through a read-only review queue API, an owner-action command API, and a frontend review UI, without implementing any backend routes, frontend screens, owner action execution, apply-promotion, or memory/canon mutation, and `PHASE8-IMPL-013-T002` (read-only review queue API contract decision) accepted `docs/roadmap/decisions/PHASE8-IMPL-013-read-only-review-queue-api-contract-decision.md` (define the read-only review queue API contract before frontend UI but implement no routes in T002; future operations are read-only GET/read-model only and review-workflow-only and never mutate queue entries, candidate records, memory, canon, project source files, raw artifacts, or indexes; safe request/query parameters, read-only response shape, mandatory evidence/provenance/uncertainty display, filtering/sorting/pagination as workflow convenience only, fail-closed error/quarantine with no repair-by-writing, owner action API separation to T003, apply-promotion/memory-canon/runtime/raw boundaries, review UI relationship to T004, and security/path safety), with `PHASE8-IMPL-013-T003` (owner action command API contract decision) ready/active and T004-T007 planned. `PHASE8-IMPL-012-T001` published the parent task record, inventory, and enrichment JSON as docs/status/planning only. `PHASE8-IMPL-012-T002` is complete as docs/decision only and accepted `docs/roadmap/decisions/PHASE8-IMPL-012-review-queue-storage-contract-decision.md`: a project-local, candidate-linked, candidate-only, review-workflow-only, evidence/provenance-backed, fail-closed review queue storage contract with a future storage root `projects/{project_id}/writer_assistant/review_queue/`, a stored queue entry shape and allowed `review_status`/`lifecycle_state` values aligned with the `PHASE8-IMPL-011-T003` queue shape, candidate linkage/integrity rules, a derived/rebuildable index contract, a storage operation boundary, and forbidden destinations. `PHASE8-IMPL-012-T003` is complete as docs/decision only and accepted `docs/roadmap/decisions/PHASE8-IMPL-012-owner-action-workflow-boundary-decision.md`: owner actions are review workflow commands and states only, not apply-promotion, not memory/canon mutation, and not approval/canon truth, with allowed owner action commands and states, a future owner action record shape, a queue entry mutation boundary, a candidate record relationship, apply-promotion and memory/canon boundaries, evidence/provenance requirements, an owner action storage boundary, a deferred review UI/API boundary, and a fail-closed/quarantine policy; it marked `PHASE8-IMPL-012-T004` (review queue storage contract tests) ready/active. The recommended child sequence is T001 parent publication (complete), T002 review queue storage contract decision (complete), T003 owner action workflow boundary decision (complete), T004 review queue storage contract tests, T005 minimal review queue storage helper if authorized, T006 review queue safety regression or conditional hardening, and T007 roadmap/status closeout. No review queue storage, queue listing/loading, owner action workflow, owner action storage, review UI/API, backend routes, frontend UI, apply-promotion, or memory/canon mutation is authorized or was added in `PHASE8-IMPL-012-T001`, `PHASE8-IMPL-012-T002`, or `PHASE8-IMPL-012-T003`. No real runtime extraction exists yet. Review queue entries remain workflow support only and queue presence is non-approval; owner review remains mandatory before anything can become approved truth. No prose generation, rewrite, continuation, automatic Storyform truth, apply-promotion, or memory/canon mutation is allowed. The candidate review gate and persistence boundary delivered by `PHASE8-IMPL-011` are implemented and tested. `PHASE8-IMPL-011-T001` published the parent task record, inventory, enrichment JSON, and roadmap/status updates as docs/status/planning only. `PHASE8-IMPL-011-T002` is complete as docs/decision only and accepted `docs/roadmap/decisions/PHASE8-IMPL-011-candidate-draft-to-record-persistence-gate-decision.md`: in-memory candidate drafts are validated input support only and require a future explicit owner-approved persistence gate before any project-local candidate record write. `PHASE8-IMPL-011-T003` is complete as docs/decision only and accepted `docs/roadmap/decisions/PHASE8-IMPL-011-review-queue-data-shape-lifecycle-decision.md`: future review queue entries are workflow support only, queue presence is non-approval, lifecycle states are review workflow states only, evidence/provenance/source locator display is mandatory, insufficient-evidence/rejected-output handling must be explicit, grouping/sorting/filtering are workflow convenience only, and owner action labels are future planning terms only. `PHASE8-IMPL-011-T004` is complete as tests-first only and added expected-red contract coverage at `tests/test_writer_assistant_core_candidate_review_gate_contract.py` for the future `backend.story_knowledge.candidate_review_gate` module and its `validate_candidate_draft_for_persistence`, `build_candidate_record_from_draft`, `build_review_queue_entry`, and `persist_candidate_record_for_review` APIs; the target pytest is expected-red with a collection `ImportError` limited to the missing future module, and candidate/orchestrator/source-evidence/OMI/project regressions pass. `PHASE8-IMPL-011-T005` is complete and created `backend/story_knowledge/candidate_review_gate.py`, a pure, standard-library-only, deterministic, candidate-only/review-pending persistence gate over existing candidate validation/persistence helpers implementing `validate_candidate_draft_for_persistence`, `build_candidate_record_from_draft`, `build_review_queue_entry`, and `persist_candidate_record_for_review`; the target contract now passes (154 tests). `PHASE8-IMPL-011-T006` is complete as a validation-only candidate review gate safety regression: the target gate contract passes (154 tests), candidate regressions pass (307 tests), orchestrator/source-evidence contracts pass (171 tests), parser/storage/adapter regressions pass (397 tests), and focused OMI/project regressions pass (109 tests); no safety gap was found, so no runtime hardening patch was needed and `candidate_review_gate.py` was left unchanged. `PHASE8-IMPL-011-T007` is complete as docs/status closeout only and recommends `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning` as the next parent (recommendation-only, not active until separately published). No review UI/API, owner action workflow, apply-promotion, memory/canon mutation, raw persistence, or runtime extraction exists. No automatic orchestrator persistence, candidate review queue storage/listing, candidate review UI/API, backend routes, frontend UI, apply-promotion, memory/canon mutation, raw artifact persistence, or real runtime extraction is authorized in T001-T007; T005 added only the narrowly tested candidate-only persistence through existing helpers, and T006/T007 changed no runtime code. Candidate drafts remain in-memory review support only, persisted records remain candidate-only/review-pending, and owner review remains mandatory before anything can become approved truth. No prose generation, rewrite, continuation, automatic Storyform truth, apply-promotion, or memory/canon mutation is allowed.
...
- Latest completed parent is `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning`, complete through `PHASE8-IMPL-012-T007 - Roadmap/status closeout`. The current active parent is `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` (active after `PHASE8-IMPL-013-T002`, docs/decision only); it is a planning/contract parent for exposing the review queue through a read-only review queue API, an owner-action command API, and a frontend review UI. `PHASE8-IMPL-013-T002` accepted `docs/roadmap/decisions/PHASE8-IMPL-013-read-only-review-queue-api-contract-decision.md` (define the read-only review queue API contract before frontend UI but implement no routes in T002; future operations are read-only GET/read-model only and review-workflow-only and never mutate queue entries, candidate records, memory, canon, project source files, raw artifacts, or indexes; safe request/query parameters, read-only response shape, mandatory evidence/provenance/uncertainty display, filtering/sorting/pagination as workflow convenience only, fail-closed error/quarantine with no repair-by-writing, owner action API separation to T003, apply-promotion/memory-canon/runtime/raw boundaries, review UI relationship to T004, and security/path safety), with `PHASE8-IMPL-013-T003` (owner action command API contract decision) ready/active and T004-T007 planned, and with no review UI/API, backend routes, frontend UI, owner action execution, apply-promotion, memory/canon mutation, or runtime extraction implemented or claimed. `PHASE8-IMPL-012` was a planning/contract parent: `PHASE8-IMPL-012-T002` accepted the review queue storage contract decision, `PHASE8-IMPL-012-T003` accepted the owner action workflow boundary decision (owner actions are review workflow commands and states only, not apply-promotion, not memory/canon mutation, not approval/canon truth), `PHASE8-IMPL-012-T004` added expected-red review queue storage contract tests for the `backend.story_knowledge.review_queue_storage` module, `PHASE8-IMPL-012-T005` created `backend/story_knowledge/review_queue_storage.py` as a pure, project-local, candidate-linked, candidate-only, review-workflow-only queue storage helper and the target review queue storage contract now passes (359 tests), `PHASE8-IMPL-012-T006` completed a validation-only review queue safety regression that found no safety gap and left the helper unchanged (target contract PASS 359 tests; candidate review gate/candidate/orchestrator/source-evidence/parser-storage-adapter/OMI regressions PASS; BookNLP/spaCy availability guard both false), and `PHASE8-IMPL-012-T007` closed the parent as docs/status closeout only. T004 added expected-red candidate persistence gate contract tests for the `backend.story_knowledge.candidate_review_gate` module, T005 implemented that pure candidate-only persistence gate helper so the target contract passes (154 tests), T006 completed a validation-only safety regression that found no safety gap and left the helper unchanged, and T007 closed the parent as docs/status closeout only. The fixture parser has TSV, `.book` JSON, token-event derivation, and raw artifact bundle builder support; the full parser contract passes. The minimal orchestrator helper is pure in-memory and the orchestrator contract passes. The review-safe fixture orchestrator is pure, in-memory, fixture-only, candidate-draft-only, non-persistent, non-canon, non-prose, and non-runtime-extraction. T002 accepted the candidate draft to candidate record persistence gate decision. T003 accepted the review queue data shape and lifecycle decision: future queue entries are workflow support only, queue presence is non-approval, lifecycle states are review workflow states only, evidence/provenance/source locator display is mandatory, and grouping/sorting/filtering are workflow convenience only. `PHASE8-IMPL-012-T005` added a pure project-local candidate-linked review-workflow-only review queue storage helper (`backend/story_knowledge/review_queue_storage.py`) with validate/build/path/write/read/list/index plus owner action record shape validation only; review UI/API, owner action workflow execution, owner decisions, apply-promotion, raw persistence, and runtime extraction remain deferred. No candidate review UI/API, owner action workflow execution, owner action storage, raw extraction artifact files, raw file write/read/list helpers, real BookNLP/spaCy install or execution, runtime extraction, routes, UI, package changes, generated prose, apply-promotion, or memory/canon mutation exists.
