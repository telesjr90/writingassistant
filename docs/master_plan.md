...
`WORKSPACE-001` through `WORKSPACE-025` define the Project Workspace Foundation planning specs. `WORKSPACE-026`, documented in `docs/roadmap/project_workspace_implementation_decision_sweep.md`, consolidates those specs into implementation-readiness decisions. Phase 7 implementation has advanced through `PHASE7-IMPL-007 - Project Overview shell`: projects can be selected, scenes/notes/materials have storage and routes, the frontend has shared owner-authored editor handling, and the workspace now includes a deterministic project overview view using existing project/list/status state. These paths remain owner-authored or owner-provided content only; no generated prose, summaries, extraction, Story Check auto-runs, model calls, OMI promotion, memory/canon mutation, or training/dataset changes were added.

The active Phase 7 frontier is now `PHASE7-IMPL-008 - OMI-guided project creation staged flow`. The next child task should start as `PHASE7-IMPL-008-T001 - OMI-guided project creation staged flow inventory and child-task plan`. `PHASE7-IMPL-008` should remain staged and owner-controlled, with candidate labels visible and no generated prose, apply-promotion, memory/canon mutation, model calls, or hidden project writes.
