...
`PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning` is complete through `PHASE8-IMPL-011-T007`. `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning` is complete through `PHASE8-IMPL-012-T007` and delivered safe queue workflow infrastructure: review queue storage and owner action record validation only, a project-local queue storage helper `backend/story_knowledge/review_queue_storage.py`, and a derived/rebuildable index. `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` is complete through `PHASE8-IMPL-013-T007`. T002 accepted `docs/roadmap/decisions/PHASE8-IMPL-013-read-only-review-queue-api-contract-decision.md`. T003 accepted `docs/roadmap/decisions/PHASE8-IMPL-013-owner-action-command-api-contract-decision.md`. T004 accepted `docs/roadmap/decisions/PHASE8-IMPL-013-review-ui-planning-boundary-decision.md`: future review UI must display workflow state, evidence, provenance, source locators, confidence as uncertainty/support strength, insufficient-evidence and rejected-output states, accessible empty/error/quarantine states, and no-promotion/no-canon warnings; owner action controls remain planning terms only and read-only viewing stays separate from command actions. T005 added expected-red review API contract tests at `tests/test_writer_assistant_review_api_contract.py` for future `backend.review_api`; frontend UI contract tests are deferred because no local frontend/component test harness exists without package changes. T006 validated that the expected-red handoff remains safe without a hardening patch. T007 closed the parent as docs/status closeout only. The parent delivered read-only review queue API contract, owner action command API contract, review UI planning boundary, and expected-red `backend.review_api` tests. No route/API/UI implementation, owner action execution, apply-promotion, memory/canon mutation, raw persistence, runtime extraction, generated prose, rewrite, continuation, automatic Storyform truth, package changes, model calls, raw artifact writes, or training/JSONL/dataset work exists or is authorized. `PHASE8-IMPL-014 - Writer Assistant Core review API implementation and tests-first route boundary` is complete through `PHASE8-IMPL-014-T007`. It delivered the pure `backend.review_api` helper module only: `backend/review_api.py` with all seven public APIs. T003 created validators and response builders; T004 integrated read-only queue helpers over `review_queue_storage`; T005 hardened owner action command validation/response shaping; T006 confirmed safety boundaries with no code change; T007 closed the parent as docs/status closeout only. `PHASE8-IMPL-015 - Review API route implementation and read-only frontend review queue surface` is complete through `PHASE8-IMPL-015-T007`. It delivered read-only review queue routes in `backend/routes/review_queue.py`, route contract tests, frontend read-only surface planning decision, and validation-only read-only safety regression with no hardening required. It did not deliver owner action execution, apply-promotion, approved memory/canon mutation, raw artifact persistence, runtime extraction, model-assisted extraction, real BookNLP/spaCy install/run/import, NCP/Subtxt/dramatica-flow runtime integration, frontend production review surface, or generated prose/prose-production paths. `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary` is complete through `PHASE8-IMPL-016-T007`. It delivered the backend review-action command route/helper, frontend owner-action review workflow/surface, and focused safety regression coverage. It did not deliver apply-promotion, approved memory/canon mutation, raw artifact persistence, runtime extraction, model-assisted extraction, real BookNLP/spaCy install/run/import, NCP/Subtxt/dramatica-flow runtime integration, or generated prose/prose-production paths. `PHASE8-IMPL-017 - Apply-promotion contract, audit log, and approved memory/canon mutation boundary` is complete through `PHASE8-IMPL-017-T007`. T001 published parent/inventory/enrichment/status alignment. T002 accepted `docs/roadmap/decisions/PHASE8-IMPL-017-apply-promotion-boundary-audit-model-decision.md`. T003 added `tests/test_writer_assistant_core_apply_promotion_contract.py`. T004 implemented `backend/story_knowledge/apply_promotion.py`, `backend/routes/apply_promotion.py`, route inclusion in `backend/main.py`, and `POST /api/projects/{project_id}/apply-promotion`. T005 implemented `submitApplyPromotion(projectId, payload)` in `frontend/src/api.js` and `frontend/src/components/ApplyPromotionConfirmation.jsx`, with source tests at `tests/test_frontend_apply_promotion_workflow_source.py`. T006 added approved memory/canon mutation safety regression coverage at `tests/test_apply_promotion_memory_canon_safety_regression.py`. T007 closed the parent as docs/status/governance only. Apply-promotion is explicit owner-confirmed only, audited, evidence/provenance/source-locator-backed, fail closed, and the only approved path from candidate/review workflow state into approved memory/canon for this parent. Candidate persistence is not canon; queue presence is not approval; confidence is not truth; raw artifacts are support data, not canon. Review queue actions do not mutate approved memory/canon. Frontend review commands remain separate from apply-promotion. Plan-building/validation performs no approved memory/canon mutation. Failed validation performs no partial mutation. Successful apply writes approved memory/canon plus applied audit record. `PHASE8-IMPL-018 - Raw artifact persistence implementation and project-local extraction artifact lifecycle` is active with `PHASE8-IMPL-018-T001` complete/PASS, `PHASE8-IMPL-018-T002` complete/PASS, `PHASE8-IMPL-018-T003` complete/PASS as expected-red contract handoff at `tests/test_writer_assistant_core_raw_artifacts_contract.py`, `PHASE8-IMPL-018-T004` complete/PASS with `backend/story_knowledge/raw_artifacts.py`, `PHASE8-IMPL-018-T005` complete/PASS with raw artifact lifecycle/provenance hardening plus `tests/test_writer_assistant_core_raw_artifact_lifecycle_contract.py`, and `PHASE8-IMPL-018-T006` complete/PASS with raw artifact safety regression coverage at `tests/test_writer_assistant_core_raw_artifact_safety_regression.py`; `PHASE8-IMPL-018-T007` is ready/active next. `PHASE8-IMPL-018-T002` accepted `docs/roadmap/decisions/PHASE8-IMPL-018-raw-artifact-persistence-boundary-manifest-model-decision.md` as docs/status decision only. `PHASE8-IMPL-018` owns raw artifact persistence lifecycle only. Raw artifacts are project-local support data only, not canon, not approved memory, not candidates by themselves, not candidates, and not training data. Raw artifact persistence must be manifest-backed by `manifest.json` with `artifact_files`, `artifact_file_id`, `relative_path`, `sha256`, `bundle_hash`, `manifest_hash`, `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs`; it must be evidence/provenance-linked, path-safe, fail-closed, quarantine-aware, and separate from apply-promotion, approved memory/canon mutation, extraction execution, model calls, BookNLP/spaCy runtime, NCP/Subtxt/dramatica-flow runtime, and generated prose. Implemented contract APIs include `validate_raw_artifact_manifest`, `build_raw_artifact_manifest`, `validate_raw_artifact_file_ref`, `raw_artifact_bundle_storage_dir`, `raw_artifact_manifest_path`, `raw_artifact_index_path`, `write_raw_artifact_bundle`, `read_raw_artifact_manifest`, `read_raw_artifact_file`, `list_raw_artifact_bundles`, `rebuild_raw_artifact_index`, `quarantine_raw_artifact_bundle`, and `compute_raw_artifact_bundle_hash`. T005 hardens valid-only list/index behavior, support data index metadata, source/evidence/provenance/source-locator linkage, stale/missing/invalid bundle behavior, quarantine_reason preservation, superseded/deleted_tombstone exclusion, duplicate/idempotency determinism, and canonical JSON `bundle_hash` behavior. T006 proves support data, safety regression, not canon, not candidates, not training data, path-safe, fail-closed, no extraction, no_runtime_extraction, no_model_calls, no_apply_promotion, no_memory_canon_mutation, no_generated_prose, quarantine safety, no approved memory/canon mutation, no candidate/review queue creation, no training artifacts, and no generated prose/prose-production behavior. `PHASE8-IMPL-019` through `PHASE8-IMPL-022` remain future MVP-required parents. Fine-tuning remains deferred after MVP. Generated prose/prose-production paths remain permanently forbidden.
