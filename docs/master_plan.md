...
`PHASE8-IMPL-019 - Guarded runtime extraction: real BookNLP/spaCy install/run/import and candidate-first extraction pipeline` is active after `PHASE8-IMPL-019-T001` docs/status publication. `PHASE8-IMPL-019-T001` is complete/PASS; `PHASE8-IMPL-019-T002` is ready/active next; `PHASE8-IMPL-019-T003` through `PHASE8-IMPL-019-T007` are planned. The parent goal is real BookNLP/spaCy install/run/import checks and guarded runtime extraction from owner-authored or owner-provided project text into raw artifact bundles and candidate-first owner review handoff, without canon mutation, model calls, automatic apply-promotion, training artifacts, or generated prose. Runtime extraction output is never canon by itself, raw tool output is never authoritative by itself, extractor confidence is not truth, future contracts must preserve `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs`, unavailable/quarantine/fail-closed states must be explicit, and no silent fallback may claim extraction succeeded. PHASE8-UX-001 remains read-only terminology/boundary reference only and does not override roadmap truth. `PHASE8-IMPL-020` through `PHASE8-IMPL-022` remain future MVP-required parents; fine-tuning remains deferred after MVP; generated prose/prose-production paths remain permanently forbidden.
