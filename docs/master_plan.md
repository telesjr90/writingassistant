...
The two P0 integrity workstreams precede UI polish; T001 is closed and T002 remains pending:

1. OMI-guided creation integrity is repaired and complete/PASS through T001A-T001D. The dedicated contract preserves exact owner input with owner provenance and linked existing OMI idea/Notes storage, records `creation_method: omi_guided`, rolls back atomically or reports `recovery_required`, preserves blank creation isolation, and passed deterministic plus disposable API/browser validation without prohibited side effects.
