...
- Latest completed parent is `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning`, complete through `PHASE8-IMPL-012-T007 - Roadmap/status closeout`. The current active parent is `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` (active after `PHASE8-IMPL-013-T003`, docs/decision only). `PHASE8-IMPL-013-T002` accepted the read-only review queue API contract, and `PHASE8-IMPL-013-T003` accepted the owner action command API contract at `docs/roadmap/decisions/PHASE8-IMPL-013-owner-action-command-api-contract-decision.md`. The T003 contract accepts only review-workflow metadata commands, rejects approval/promotion/apply-promotion/memory-canon/generated-prose/runtime/raw/training/JSONL/unknown commands, requires actor/audit/evidence/provenance and no-promotion/no-memory-canon request/response fields, and fails closed. `PHASE8-IMPL-013-T004` is ready/active; T005-T007 are planned. No review UI/API, backend routes, frontend UI, owner action execution, apply-promotion, memory/canon mutation, runtime extraction, raw artifact persistence, generated prose, package changes, model calls, or training/JSONL/dataset work is implemented or claimed.
