...
- Latest completed parent is `PHASE8-IMPL-010 - Writer Assistant Core extraction orchestration planning and review-safe pipeline boundary`, complete through `PHASE8-IMPL-010-T007 - Roadmap/status closeout`. The active frontier is `PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning`, active after `PHASE8-IMPL-011-T006`, with `PHASE8-IMPL-011-T007` ready/active. T004 added expected-red candidate persistence gate contract tests for the `backend.story_knowledge.candidate_review_gate` module, T005 implemented that pure candidate-only persistence gate helper so the target contract passes (154 tests), and T006 completed a validation-only safety regression that found no safety gap and left the helper unchanged. The fixture parser has TSV, `.book` JSON, token-event derivation, and raw artifact bundle builder support; the full parser contract passes. The minimal orchestrator helper is pure in-memory and the orchestrator contract passes. The review-safe fixture orchestrator is pure, in-memory, fixture-only, candidate-draft-only, non-persistent, non-canon, non-prose, and non-runtime-extraction. T002 accepted the candidate draft to candidate record persistence gate decision. T003 accepted the review queue data shape and lifecycle decision: future queue entries are workflow support only, queue presence is non-approval, lifecycle states are review workflow states only, evidence/provenance/source locator display is mandatory, and grouping/sorting/filtering are workflow convenience only. Candidate persistence implementation, review queue implementation, review UI/API, owner decisions, apply-promotion, raw persistence, and runtime extraction remain deferred. No candidate persistence, candidate review queue, candidate review UI/API, raw extraction artifact files, raw file write/read/list helpers, real BookNLP/spaCy install or execution, runtime extraction, routes, UI, package changes, generated prose, apply-promotion, or memory/canon mutation exists.
