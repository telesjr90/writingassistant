...
`WORKSPACE-001` through `WORKSPACE-025` define the Project Workspace Foundation planning specs. `WORKSPACE-026`, documented in `docs/roadmap/project_workspace_implementation_decision_sweep.md`, consolidates those specs into implementation-readiness decisions. Phase 7 implementation has advanced through `PHASE7-IMPL-009 - Memory / Canon shell (approved-only empty states)`: projects can be selected, scenes/notes/materials have storage and routes, the frontend has shared owner-authored editor handling, the workspace includes a deterministic project overview view, OMI-guided project creation has a frontend-transient owner-controlled staged shell, and the workspace now includes a read-only Memory / Canon shell with approved-only empty states and App/Nav integration. These paths remain owner-authored or owner-provided content only; no generated prose, summaries, extraction, Story Check auto-runs, model calls, OMI promotion, apply-promotion, memory/canon mutation, backend approved-memory routes/helpers, frontend approved-memory API helpers, hidden pre-confirmation project writes, staged backend storage/routes/API helpers, or training/dataset changes were added.

The active Phase 7 frontier is now `PHASE7-IMPL-010 - Workspace Validation / Browser and Manual Smoke`. `PHASE7-IMPL-010` should remain validation-only and must not add runtime feature scope, apply-promotion, memory/canon mutation, generated prose, model calls, extraction, or training/dataset work unless explicitly authorized by a separate roadmap task.
