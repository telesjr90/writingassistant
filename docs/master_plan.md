...
The published Phase 7 parent sequence (`PHASE7-IMPL-001` through `PHASE7-IMPL-010`) is complete. Writer Assistant Core is the current roadmap frontier. `PHASE8-IMPL-001` is complete through `PHASE8-IMPL-001-T007`. `PHASE8-IMPL-002` is complete through `PHASE8-IMPL-002-T007`. `PHASE8-IMPL-003` is complete through `PHASE8-IMPL-003-T007`. `PHASE8-IMPL-004` is the active parent through `PHASE8-IMPL-004-T005`.

Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, and derived candidate index helpers plus index safety regression tests in `PHASE8-IMPL-004` through T005. The app still does not extract story knowledge at runtime. The app still has no extraction/model route, frontend extraction/review UI, apply-promotion, or memory/canon mutation. Candidate-only JSON persistence exists under `writer_assistant/candidates/*.json` and derived index helpers exist in `backend/story_knowledge/candidate_index.py` through helper functions only; no backend routes or frontend review UI consume it yet. `PHASE8-IMPL-004` is active for conditional T006 docs/status validation and T007 closeout before routes/UI/extraction/model expansion.
...
- Latest completed frontier parent is `PHASE8-IMPL-003 - Writer Assistant Core candidate storage read/write contract and candidate-only persistence`; it is complete through `PHASE8-IMPL-003-T007 - Roadmap/status closeout`. Active parent is `PHASE8-IMPL-004 - Writer Assistant Core candidate index contract and derived index helpers`; it is active through `PHASE8-IMPL-004-T005 - Index safety and stale/corrupt regression tests`. Next child is `PHASE8-IMPL-004-T006 - Index safety repair or hardening` (conditional docs/status validation only). Prior completed parents are `PHASE8-IMPL-002` and `PHASE8-IMPL-001`.
