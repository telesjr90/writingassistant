...
- Latest completed parent is `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`, complete through `PHASE8-IMPL-009-T007 - Roadmap/status closeout`. Active frontier is `PHASE8-IMPL-010 - Writer Assistant Core extraction orchestration planning and review-safe pipeline boundary`, complete through `PHASE8-IMPL-010-T004`; `PHASE8-IMPL-010-T005` is ready/active. The fixture parser has TSV, `.book` JSON, token-event derivation, and raw artifact bundle builder support; the full parser contract passes. The minimal orchestrator helper is pure in-memory and the orchestrator contract passes. No raw extraction artifact files exist, no raw file write/read/list helpers exist, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists.
