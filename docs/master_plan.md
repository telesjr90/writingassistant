...
Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, derived candidate index contract/helpers/safety regressions in `PHASE8-IMPL-004`, and tool evaluation/extraction strategy in `PHASE8-IMPL-005`. The app still does not extract story knowledge at runtime. The app still has no extraction/model route, frontend extraction/review UI, apply-promotion, or memory/canon mutation. Candidate-only JSON persistence exists under `writer_assistant/candidates/*.json` and derived index helpers exist in `backend/story_knowledge/candidate_index.py` through helper functions only; no backend routes or frontend review UI consume them yet. `PHASE8-IMPL-005` accepted spaCy-first local deterministic/rule-assisted candidate extraction foundation as the first extraction strategy. dramatica-flow remains `reference-only` for analysis-pattern inspiration; Narrative Context Protocol remains `reference-only` and future approved-context import/export candidate; Subtxt docs remain `reference-only` and future semantic rubric candidate. segram, BookNLP, GLiNER, LangExtract, and Renard are deferred from the first implementation slice; LangExtract requires a local/Ollama-only privacy policy before adoption, and Renard requires GPL/license review before adoption. Scope was narrowed to exactly nine approved tools/references. Broader candidates (CoreNLP/OpenIE/SUTime, AI-Reader-V2, narrative-blueprint, NovelClaw, NotebookLM workflow, llm_finetuning, ai-llm-project-file-structure-template) remain out of scope unless a later owner-approved parent reintroduces them. Extraction implementation remains future work under the recommended next parent, `PHASE8-IMPL-006 - Writer Assistant Core analysis-only candidate extraction architecture and spaCy-first local extraction foundation`. No prose generation, rewrite, continuation, apply-promotion, memory/canon mutation, runtime extraction, package/dependency change, or model call was added by PHASE8-IMPL-005.
...
- Latest completed frontier parent is `PHASE8-IMPL-005 - Writer Assistant Core tool evaluation and extraction strategy decision`; it is complete through `PHASE8-IMPL-005-T007 - Roadmap/status closeout`. Recommended next parent is `PHASE8-IMPL-006 - Writer Assistant Core analysis-only candidate extraction architecture and spaCy-first local extraction foundation`, pending separate owner-approved publication.
