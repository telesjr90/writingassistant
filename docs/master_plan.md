...
`PHASE8-IMPL-019 - Guarded runtime extraction: real BookNLP/spaCy install/run/import and candidate-first extraction pipeline` is active after `PHASE8-IMPL-019-T004` minimal guarded dependency availability and import/run probe implementation. `PHASE8-IMPL-019-T001` is complete/PASS; `PHASE8-IMPL-019-T002` is complete/PASS with the accepted Guarded runtime extraction boundary and environment model decision at `docs/roadmap/decisions/PHASE8-IMPL-019-guarded-runtime-extraction-boundary-environment-model-decision.md`; `PHASE8-IMPL-019-T003` is complete/PASS as expected-red contract coverage; `PHASE8-IMPL-019-T004` is complete/PASS with `backend/story_knowledge/runtime_extraction.py`; `PHASE8-IMPL-019-T005` is ready/active next; and `PHASE8-IMPL-019-T006` through `PHASE8-IMPL-019-T007` are planned. The parent goal is real BookNLP/spaCy install/run/import checks and guarded runtime extraction from owner-authored or owner-provided project text into raw artifact bundles and candidate-first owner review handoff, without canon mutation, model calls, automatic apply-promotion, training artifacts, or generated prose. Runtime extraction output is never canon by itself, raw tool output is never authoritative by itself, confidence is not truth, future contracts must preserve `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs`, availability/unavailable/quarantine/fail-closed states must be explicit, and no silent fallback may claim extraction succeeded. T004 implements disabled-by-default environment validation, explicit BookNLP/spaCy availability states, request validation, deterministic side-effect-free plans, transient probe results, draft handoff/quarantine objects, and no extraction success fallback; it does not install dependencies, edit package files, add routes/UI, persist candidates/review entries, mutate canon/memory, call models, create training artifacts, run full runtime extraction over project text, or generate prose. PHASE8-UX-001 remains read-only terminology/boundary reference only and does not override roadmap truth. `PHASE8-IMPL-020` through `PHASE8-IMPL-022` remain future MVP-required parents; fine-tuning remains deferred after MVP; generated prose/prose-production paths remain permanently forbidden.
