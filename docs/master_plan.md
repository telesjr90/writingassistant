...
Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, derived candidate index contract/helpers/safety regressions in `PHASE8-IMPL-004`, tool evaluation/extraction strategy in `PHASE8-IMPL-005`, the evidence-first extraction foundation plus BookNLP-ready adapter strategy in `PHASE8-IMPL-006`, and the mocked BookNLP adapter contract implementation and normalization foundation in `PHASE8-IMPL-007`. `PHASE8-IMPL-007` is complete through `PHASE8-IMPL-007-T007`. It delivered read-only official source inventory using `.external_sources/`, implementation decisions, `backend/story_knowledge/booknlp_adapter_contract.py`, all six public APIs, manifest validation, raw artifact bundle validation, mocked entity/quote/event normalization, the mocked candidate draft builder, fail-closed behavior, and source/evidence/provenance/raw-ref integration. All adapter behavior remains pure, standard-library-only, and bounded to in-memory mocked dictionaries. The app still does not extract story knowledge at runtime. The app still has no extraction/model route, frontend extraction/review UI, apply-promotion, production BookNLP runtime adapter, raw output storage helper, real BookNLP file parser, or memory/canon mutation. BookNLP and spaCy have not been installed or run. No prose generation, rewrite, continuation, automatic Storyform truth, apply-promotion, memory/canon mutation, runtime extraction, package/dependency change, or model call is allowed unless a later child explicitly authorizes bounded work. `PHASE8-IMPL-008` is now active as the raw extraction artifact storage and BookNLP fixture parser contract parent. `PHASE8-IMPL-008-T001` published the parent task record at `docs/roadmap/tasks/PHASE8-IMPL-008.md`, the inventory at `docs/roadmap/inventory/PHASE8-IMPL-008.md`, the enrichment JSON at `docs/roadmap/enrichment/PHASE8-IMPL-008.enrichment.json`, and the roadmap/status updates. `PHASE8-IMPL-008-T002` completed the raw extraction artifact storage contract decision at `docs/roadmap/decisions/PHASE8-IMPL-008-raw-extraction-artifact-storage-contract-decision.md`, accepting `writer_assistant/extractions/{tool_name}/{run_id}/`, `booknlp` as the first tool folder, strict tool/run ID safety, manifest shape and policy flags, raw/derived artifact kinds, and tests-only T003 handoff. T001/T002 did not implement extraction, raw extraction artifact helpers, raw extraction artifact files, or a BookNLP fixture parser, and did not change the existing mocked adapter contract module or test. The active child is `PHASE8-IMPL-008-T003` (ready/active); `PHASE8-IMPL-008-T004` through `PHASE8-IMPL-008-T007` are planned/draft. No raw extraction artifact storage helpers or files exist yet, no BookNLP fixture parser exists yet, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists. Recommended next parent, not active until separately published: `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`.
...
- Latest completed frontier parent is `PHASE8-IMPL-007 - Writer Assistant Core BookNLP adapter contract implementation and mocked normalization foundation`; it is complete through `PHASE8-IMPL-007-T007 - Roadmap/status closeout`. Latest active parent is `PHASE8-IMPL-008 - Writer Assistant Core raw extraction artifact storage and BookNLP fixture parser contract`; it is active with `PHASE8-IMPL-008-T001 - Publish raw extraction artifact storage and BookNLP fixture parser contract parent` complete and `PHASE8-IMPL-008-T002 - Raw extraction artifact storage contract decision` complete. The active child is `PHASE8-IMPL-008-T003 - Extraction artifact storage path and manifest contract tests` (ready/active); `PHASE8-IMPL-008-T004` through `PHASE8-IMPL-008-T007` are planned/draft. No raw extraction artifact storage helpers or files exist yet, no BookNLP fixture parser exists yet, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists. Recommended next parent is `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`, recommended only and not active.
