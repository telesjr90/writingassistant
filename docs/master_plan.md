...
`WORKSPACE-001` through `WORKSPACE-025` define the Project Workspace Foundation planning specs. `WORKSPACE-026`, documented in `docs/roadmap/project_workspace_implementation_decision_sweep.md`, consolidates those specs into implementation-readiness decisions. Phase 7 implementation has advanced through `PHASE7-IMPL-008 - OMI-guided project creation staged flow`: projects can be selected, scenes/notes/materials have storage and routes, the frontend has shared owner-authored editor handling, the workspace includes a deterministic project overview view, and OMI-guided project creation now has a frontend-transient owner-controlled staged shell. These paths remain owner-authored or owner-provided content only; no generated prose, summaries, extraction, Story Check auto-runs, model calls, OMI promotion, apply-promotion, memory/canon mutation, hidden pre-confirmation project writes, staged backend storage/routes/API helpers, or training/dataset changes were added.

The active Phase 7 frontier is now `PHASE7-IMPL-009 - Memory / Canon shell (approved-only empty states)`. `PHASE7-IMPL-009` should remain approved-only and shell/empty-state oriented, with no automatic promotion, model calls, generated prose, or memory/canon mutation.
