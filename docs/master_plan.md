...
Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, derived candidate index contract/helpers/safety regressions in `PHASE8-IMPL-004`, tool evaluation/extraction strategy in `PHASE8-IMPL-005`, and the evidence-first extraction foundation plus BookNLP-ready adapter strategy in `PHASE8-IMPL-006`. `PHASE8-IMPL-006` is complete through `PHASE8-IMPL-006-T007`. It accepted the app-owned evidence/source-map contract at `docs/roadmap/decisions/PHASE8-IMPL-006-evidence-source-map-contract-decision.md`, added source/evidence contract tests in `tests/test_writer_assistant_core_source_evidence_contract.py`, and added pure validation helpers in `backend/story_knowledge/source_map.py` and `backend/story_knowledge/evidence.py` for source document references, source segments, source maps, source locators, evidence records, extraction run provenance, and raw output references. It also accepted the BookNLP-ready raw output and adapter contract at `docs/roadmap/decisions/PHASE8-IMPL-006-booknlp-ready-raw-output-adapter-contract-decision.md` and added adapter contract tests in `tests/test_writer_assistant_core_booknlp_adapter_contract.py` for `backend.story_knowledge.booknlp_adapter_contract`. `PHASE8-IMPL-007 - Writer Assistant Core BookNLP adapter contract implementation and mocked normalization foundation` is now active after `PHASE8-IMPL-006`. `PHASE8-IMPL-007-T001` published the parent task record, inventory, enrichment JSON, and roadmap/status updates; confirmed that `.external_sources/booknlp`, `.external_sources/dramatica-flow`, `.external_sources/narrative-context-protocol`, and `.external_sources/subtxt-docs` exist locally and are protected from commit via `.git/info/exclude`. `PHASE8-IMPL-007-T002` completed the read-only official source inventory and implementation refresh decision, creating `docs/roadmap/inventory/PHASE8-IMPL-007-official-source-inventory.md` and `docs/roadmap/decisions/PHASE8-IMPL-007-implementation-source-refresh-decision.md`. T002 confirmed real BookNLP output field names, refined `booknlp_events` as app-owned derived support from `.tokens.event`, and kept dramatica-flow, NCP, and Subtxt docs out of direct implementation. `PHASE8-IMPL-007-T003` completed the implementation decision at `docs/roadmap/decisions/PHASE8-IMPL-007-booknlp-adapter-implementation-decision.md`, accepting T004 validators/API symbols first, T005 mocked normalizers, and T006 candidate draft builder/boundary hardening. `PHASE8-IMPL-007-T004` created `backend/story_knowledge/booknlp_adapter_contract.py` as a pure mocked standard-library-only module, exposed all six public APIs, implemented run manifest and raw artifact bundle validators, supplied minimal in-memory draft shaping required by current contract tests, and made `tests/test_writer_assistant_core_booknlp_adapter_contract.py` pass. `PHASE8-IMPL-007-T005` completed as validation-only after reviewing mocked entity/quote/event normalization and confirming no repair gap. `PHASE8-IMPL-007-T006` completed as validation-only after reviewing the candidate draft builder, fail-closed behavior, and boundary hardening scope and confirming no runtime/test repair gap. The next child is `PHASE8-IMPL-007-T007` for roadmap/status closeout. The app still does not extract story knowledge at runtime. The app still has no extraction/model route, frontend extraction/review UI, apply-promotion, production BookNLP runtime adapter, or memory/canon mutation. BookNLP and spaCy have not been installed or run. No prose generation, rewrite, continuation, automatic Storyform truth, apply-promotion, memory/canon mutation, runtime extraction, package/dependency change, or model call is allowed unless a later child explicitly authorizes bounded work.
...
- Latest completed frontier parent is `PHASE8-IMPL-005 - Writer Assistant Core tool evaluation and extraction strategy decision`; it is complete through `PHASE8-IMPL-005-T007 - Roadmap/status closeout`. Active parent is `PHASE8-IMPL-006 - Writer Assistant Core evidence-first extraction foundation and BookNLP-ready adapter strategy`; last completed child is `PHASE8-IMPL-006-T005 - BookNLP-ready raw output and adapter contract decision`; next child is `PHASE8-IMPL-006-T006 - BookNLP-ready adapter contract tests`.
