...
- Active frontier parent is `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`; `PHASE8-IMPL-009-T001`, `PHASE8-IMPL-009-T002`, and `PHASE8-IMPL-009-T003` are complete, and `PHASE8-IMPL-009-T004 - Book JSON parsing and token-event derivation implementation` is ready/active. Latest completed parent remains `PHASE8-IMPL-008 - Writer Assistant Core raw extraction artifact storage and BookNLP fixture parser contract`, complete through `PHASE8-IMPL-008-T007 - Roadmap/status closeout`. The fixture parser now has TSV helpers only; no raw extraction artifact files exist, no raw file write/read/list helpers exist, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists.
