...
Writer Assistant Core has completed schema constants in `PHASE8-IMPL-001`, candidate record validation and storage path contract helpers in `PHASE8-IMPL-002`, candidate-only JSON write/read/list persistence helpers in `PHASE8-IMPL-003`, derived candidate index contract/helpers/safety regressions in `PHASE8-IMPL-004`, and tool evaluation/extraction strategy in `PHASE8-IMPL-005`. `PHASE8-IMPL-006` is now the active parent after the completed tool strategy: Writer Assistant Core evidence-first extraction foundation and BookNLP-ready adapter strategy. `PHASE8-IMPL-006-T002` accepted the app-owned evidence/source-map contract at `docs/roadmap/decisions/PHASE8-IMPL-006-evidence-source-map-contract-decision.md`, covering project-local source document identity, source maps, source locators, primary character-offset evidence spans over exact UTF-8 decoded snapshots, source hashes, Evidence Ledger/provenance shape, raw output boundaries, BookNLP-ready mapping assumptions, and candidate normalization guardrails. `PHASE8-IMPL-006-T003` added tests-first coverage in `tests/test_writer_assistant_core_source_evidence_contract.py` for source-map/evidence helper APIs. `PHASE8-IMPL-006-T004` added pure validation helpers in `backend/story_knowledge/source_map.py` and `backend/story_knowledge/evidence.py`; the source/evidence contract tests now pass. `PHASE8-IMPL-006-T005` accepted the BookNLP-ready raw output and adapter contract at `docs/roadmap/decisions/PHASE8-IMPL-006-booknlp-ready-raw-output-adapter-contract-decision.md`, covering future raw artifact kinds, storage/manifest boundaries, mocked fixtures, adapter normalization, candidate/evidence mapping, and fail-closed behavior. `PHASE8-IMPL-006-T006` added tests-first BookNLP-ready adapter contract coverage in `tests/test_writer_assistant_core_booknlp_adapter_contract.py` for the future pure module `backend.story_knowledge.booknlp_adapter_contract`; the targeted pytest is expected red until that module exists. The app still does not extract story knowledge at runtime. The app still has no extraction/model route, frontend extraction/review UI, apply-promotion, production BookNLP adapter, or memory/canon mutation. Candidate-only JSON persistence exists under `writer_assistant/candidates/*.json` and derived index helpers exist in `backend/story_knowledge/candidate_index.py` through helper functions only; no backend routes or frontend review UI consume them yet. The active direction is evidence-first + BookNLP-ready, not spaCy-only. BookNLP is framed as the first serious literary extractor candidate after source maps/provenance exist; spaCy is framed as a possible lightweight local baseline/support option. dramatica-flow remains reference-only or future wrapped analysis-only rubric source; Narrative Context Protocol remains reference-only and future approved-context import-export target; Subtxt docs remain reference-only and future semantic rubric/guardrail source. No extraction implementation is claimed yet. No prose generation, rewrite, continuation, automatic Storyform truth, apply-promotion, memory/canon mutation, runtime extraction, package/dependency change, or model call is allowed in this parent unless a later child explicitly authorizes bounded work.
