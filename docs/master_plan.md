...
- Latest completed parent is `PHASE8-IMPL-015 - Review API route implementation and read-only frontend review queue surface`, complete through `PHASE8-IMPL-015-T007 - Roadmap/status closeout`. Current active parent is `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary`; `PHASE8-IMPL-016-T001` is complete/PASS and `PHASE8-IMPL-016-T002` is ready/active next. `PHASE8-IMPL-015` delivered read-only review queue routes in `backend/routes/review_queue.py`, route contract tests, frontend read-only surface planning decision, and validation-only read-only safety regression. It did not deliver owner action execution, apply-promotion, approved memory/canon mutation, raw artifact persistence, runtime extraction, model-assisted extraction, real BookNLP/spaCy install/run/import, NCP/Subtxt/dramatica-flow runtime integration, frontend production review surface, or generated prose/prose-production paths. Future MVP-required parents remain `PHASE8-IMPL-017` through `PHASE8-IMPL-022`. Fine-tuning remains deferred after MVP. Generated prose/prose-production paths remain permanently forbidden.
