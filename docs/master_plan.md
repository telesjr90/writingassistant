...
- Latest completed frontier parent is `PHASE8-IMPL-008 - Writer Assistant Core raw extraction artifact storage and BookNLP fixture parser contract`; it is complete through `PHASE8-IMPL-008-T007 - Roadmap/status closeout`. No active child is published after T007; next parent publication is pending owner approval. No raw extraction artifact files exist, no raw file write/read/list helpers exist, no BookNLP fixture parser exists yet, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists. Recommended next parent is `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`, recommended only and not active.
