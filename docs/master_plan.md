...
`PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning` is complete through `PHASE8-IMPL-011-T007`. `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning` is complete through `PHASE8-IMPL-012-T007` and delivered safe queue workflow infrastructure: review queue storage and owner action record validation only, a project-local queue storage helper `backend/story_knowledge/review_queue_storage.py`, and a derived/rebuildable index. `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` is complete through `PHASE8-IMPL-013-T007`. `PHASE8-IMPL-014 - Writer Assistant Core review API implementation and tests-first route boundary` is complete through `PHASE8-IMPL-014-T007`. `PHASE8-IMPL-015 - Review API route implementation and read-only frontend review queue surface` is complete through `PHASE8-IMPL-015-T007`. `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary` is complete through `PHASE8-IMPL-016-T007`. `PHASE8-IMPL-017 - Apply-promotion contract, audit log, and approved memory/canon mutation boundary` is complete through `PHASE8-IMPL-017-T007`; apply-promotion is explicit owner-confirmed only, audited, evidence/provenance/source-locator-backed, fail closed, and the only approved path from candidate/review workflow state into approved memory/canon for that parent. `PHASE8-IMPL-018 - Raw artifact persistence implementation and project-local extraction artifact lifecycle` is complete/PASS through `PHASE8-IMPL-018-T007`. `PHASE8-IMPL-018-T002` accepted `docs/roadmap/decisions/PHASE8-IMPL-018-raw-artifact-persistence-boundary-manifest-model-decision.md`; T003 delivered `tests/test_writer_assistant_core_raw_artifacts_contract.py`; T004 delivered `backend/story_knowledge/raw_artifacts.py`; T005 delivered lifecycle/provenance hardening plus `tests/test_writer_assistant_core_raw_artifact_lifecycle_contract.py`; T006 delivered `tests/test_writer_assistant_core_raw_artifact_safety_regression.py`; and T007 closed the parent as docs/status/governance only. Raw artifacts are project-local support data only, not canon, not approved memory, not candidates by themselves, not candidates, and not training data. Raw artifact persistence is manifest-backed by `manifest.json` with `artifact_files`, `artifact_file_id`, `relative_path`, `sha256`, `bundle_hash`, `manifest_hash`, `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs`; it is evidence/provenance-linked, path-safe, fail-closed, quarantine-aware, and separate from apply-promotion, approved memory/canon mutation, extraction execution, model calls, BookNLP/spaCy runtime, NCP/Subtxt/dramatica-flow runtime, and generated prose. Implemented APIs include `validate_raw_artifact_manifest`, `build_raw_artifact_manifest`, `validate_raw_artifact_file_ref`, `raw_artifact_bundle_storage_dir`, `raw_artifact_manifest_path`, `raw_artifact_index_path`, `write_raw_artifact_bundle`, `read_raw_artifact_manifest`, `read_raw_artifact_file`, `list_raw_artifact_bundles`, `rebuild_raw_artifact_index`, `quarantine_raw_artifact_bundle`, and `compute_raw_artifact_bundle_hash`. Delivered behavior includes manifest-backed raw artifact bundles, project-local path-safe storage helpers, valid-only default listing/indexing, derived/rebuildable support-data indexes, source/evidence/provenance/source-locator refs, quarantine handling, superseded/deleted_tombstone exclusion by default, duplicate/idempotency handling, canonical deterministic bundle hashing, and fail-closed validation. T006/T007 prove support data, safety regression, not canon, not candidates, not training data, path-safe, fail-closed, no extraction, no_runtime_extraction, no_booknlp_spacy_runtime, no_model_calls, no_apply_promotion, no_memory_canon_mutation, no_generated_prose, quarantine safety, no approved memory/canon mutation, no candidate/review queue creation, no training artifacts, and no generated prose/prose-production behavior. Explicit non-deliveries: no runtime extraction, no real BookNLP/spaCy install/run/import, no model-assisted extraction, no NCP/Subtxt/dramatica-flow runtime, no routes, no UI, no apply-promotion changes, no approved memory/canon mutation beyond the existing `PHASE8-IMPL-017` path, no candidate/review queue creation, no training/JSONL/dataset/model artifacts, and no generated prose. `PHASE8-IMPL-019` is active after T005 guarded request/raw artifact handoff implementation; T006 is ready/active next. `PHASE8-IMPL-020` through `PHASE8-IMPL-022` remain future MVP-required parents. Fine-tuning remains deferred after MVP. Generated prose/prose-production paths remain permanently forbidden.

`PHASE8-IMPL-019 - Guarded runtime extraction: real BookNLP/spaCy install/run/import and candidate-first extraction pipeline` is active after `PHASE8-IMPL-019-T006` runtime extraction safety regression. `PHASE8-IMPL-019-T001` is complete/PASS; `PHASE8-IMPL-019-T002` is complete/PASS with the accepted Guarded runtime extraction boundary and environment model decision at `docs/roadmap/decisions/PHASE8-IMPL-019-guarded-runtime-extraction-boundary-environment-model-decision.md`; `PHASE8-IMPL-019-T003` is complete/PASS as expected-red contract coverage; `PHASE8-IMPL-019-T004` is complete/PASS with `backend/story_knowledge/runtime_extraction.py`; `PHASE8-IMPL-019-T005` is complete/PASS with guarded raw artifact persistence handoff; `PHASE8-IMPL-019-T006` is complete/PASS with focused safety regression coverage and minimal helper hardening; and `PHASE8-IMPL-019-T007` is ready/active next. The parent goal is real BookNLP/spaCy install/run/import checks and guarded runtime extraction from owner-authored or owner-provided project text into raw artifact bundles and candidate-first owner review handoff, without canon mutation, model calls, automatic apply-promotion, training artifacts, or generated prose. Runtime extraction output is never canon by itself, raw tool output is never authoritative by itself, confidence is not truth, future contracts must preserve `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs`, availability/unavailable/quarantine/fail-closed states must be explicit, and no silent fallback may claim extraction succeeded. T005 persists valid support-data-only raw artifact bundles through PHASE8-IMPL-018 helpers and rejects or quarantines unsafe, malformed, incomplete, or unsupported outputs. T006 proves those runtime extraction helpers fail closed for unsafe source paths, unsafe ids, unsupported/non-owner source claims, missing refs, invalid source locators, forbidden artifact/action types, forbidden raw artifact destinations, forbidden payload markers, malformed output, unavailable/quarantine states, no silent success, and raw artifact persistence failures. T006 does not install dependencies, edit package files, add routes/UI, persist candidates/review entries, mutate canon/memory, call models, create training artifacts, run full runtime extraction over project text, or generate prose. PHASE8-UX-001 remains read-only terminology/boundary reference only and does not override roadmap truth. `PHASE8-IMPL-020` through `PHASE8-IMPL-022` remain future MVP-required parents; fine-tuning remains deferred after MVP; generated prose/prose-production paths remain permanently forbidden.
