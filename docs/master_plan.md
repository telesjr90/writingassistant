...
`PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning` is complete through `PHASE8-IMPL-011-T007`. `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning` is complete through `PHASE8-IMPL-012-T007` and delivered safe queue workflow infrastructure: review queue storage and owner action record validation only, a project-local queue storage helper `backend/story_knowledge/review_queue_storage.py`, and a derived/rebuildable index. `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` is complete through `PHASE8-IMPL-013-T007`. `PHASE8-IMPL-014 - Writer Assistant Core review API implementation and tests-first route boundary` is complete through `PHASE8-IMPL-014-T007`. `PHASE8-IMPL-015 - Review API route implementation and read-only frontend review queue surface` is complete through `PHASE8-IMPL-015-T007`. `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary` is complete through `PHASE8-IMPL-016-T007`. `PHASE8-IMPL-017 - Apply-promotion contract, audit log, and approved memory/canon mutation boundary` is complete through `PHASE8-IMPL-017-T007`; apply-promotion is explicit owner-confirmed only, audited, evidence/provenance/source-locator-backed, fail closed, and the only approved path from candidate/review workflow state into approved memory/canon for that parent. `PHASE8-IMPL-018 - Raw artifact persistence implementation and project-local extraction artifact lifecycle` is complete/PASS through `PHASE8-IMPL-018-T007`. `PHASE8-IMPL-019 - Guarded runtime extraction: real BookNLP/spaCy install/run/import and candidate-first extraction pipeline` is complete/PASS through `PHASE8-IMPL-019-T007`. `PHASE8-IMPL-020 - Model-assisted evidence-backed extraction and diagnostic workflow` is complete/PASS through `PHASE8-IMPL-020-T007`. `PHASE8-IMPL-021 - Analysis-only NCP/Subtxt/dramatica-flow runtime integration` is active; `PHASE8-IMPL-021-T001` is complete/PASS as docs/status publication only, `PHASE8-IMPL-021-T002` is complete/PASS as docs/decision only, `PHASE8-IMPL-021-T003` is complete/PASS as tests-first expected-red contract tests only, `PHASE8-IMPL-021-T004` is complete/PASS as minimal pure analysis runtime request/allowlist guard helper implementation at `backend/story_knowledge/analysis_runtime_integration.py`, `PHASE8-IMPL-021-T005` is complete/PASS as evidence-backed runtime output handoff implementation, `PHASE8-IMPL-021-T006` is complete/PASS as analysis runtime integration safety regression with `tests/test_writer_assistant_core_analysis_runtime_integration_safety_regression.py` and minimal helper hardening, and `PHASE8-IMPL-021-T007` is ready/active next. `PHASE8-IMPL-022` remains future MVP-required. Fine-tuning remains deferred after MVP. Generated prose/prose-production paths remain permanently forbidden.

`PHASE8-IMPL-021` is the active analysis-only NCP/Subtxt/dramatica-flow runtime integration parent. `PHASE8-IMPL-021-T002` accepted the docs/decision-only analysis-only runtime integration boundary and audited allowlist decision at `docs/roadmap/decisions/PHASE8-IMPL-021-analysis-only-runtime-integration-boundary-allowlist-decision.md`. `PHASE8-IMPL-021-T003` added tests-first expected-red contract coverage at `tests/test_writer_assistant_core_analysis_runtime_integration_contract.py`. `PHASE8-IMPL-021-T004` implemented the minimal pure analysis runtime request/allowlist guard helper at `backend/story_knowledge/analysis_runtime_integration.py` with `validate_analysis_runtime_allowlist_record`, `validate_analysis_runtime_request`, `build_analysis_runtime_plan`, `validate_analysis_runtime_output`, `build_analysis_runtime_candidate_support`, `build_analysis_runtime_diagnostic_questions`, `quarantine_analysis_runtime_output`, and `run_guarded_analysis_runtime_integration`. `PHASE8-IMPL-021-T005` added evidence-backed runtime output handoff helpers `build_analysis_runtime_candidate_observation_handoff`, `build_analysis_runtime_diagnostic_handoff`, `build_analysis_runtime_review_handoff`, and `validate_analysis_runtime_review_handoff`, with focused coverage in `tests/test_writer_assistant_core_analysis_runtime_integration_handoff_contract.py`. `PHASE8-IMPL-021-T006` added safety regression coverage at `tests/test_writer_assistant_core_analysis_runtime_integration_safety_regression.py` and minimal helper hardening for unsafe refs, missing refs, nested forbidden outputs, prose intents, and non-execution flags. `PHASE8-IMPL-021-T007` is ready/active next. NCP is structured context interchange only. Subtxt is rubric/diagnostic guidance only. dramatica-flow is usable only through an audited allowlist that blocks prose paths, outline paths, rewrite, continuation, draft, revision, polish, improvement, expansion, style imitation, chapter generation, write/revise, export-as-prose, and story-prose paths. Runtime handoffs must remain candidate-first, evidence-backed, provenance-backed, source-locator-backed when available, and owner-review-required; `source_refs`, `evidence_refs`, `provenance_refs`, and `source_locator_refs` are required when available. T006 covers unsafe_path, allowlist_missing, allowlist_denied, unsupported_tool, unsupported_action, missing_source_refs, missing_evidence_refs, missing_provenance_refs, missing_source_locator_refs, source_locator_invalid, forbidden_output_type, refused_no_prose, blocked_request, quarantined, and fail_closed. Confidence is not truth; tool output is not canon; tool output is not truth; no automatic canon; no apply-promotion; no memory/canon mutation; no training artifacts; no generated prose; no rewrite; no continuation; no outline; fail closed; no silent fallback; queue presence is not approval; and candidate persistence is not canon remain required. T006 added no NCP/Subtxt/dramatica-flow execution, no routes, no UI, no frontend changes, no package/dependency changes, no model/Ollama calls, no network/subprocess calls, no persistence, no canon mutation, no apply-promotion, no training artifacts, and no generated prose behavior.
