...
5. After training, export GGUF, load into Ollama as `dramatica-analyst:8b`, and swap the backend model default only after a non-smoke model passes evaluation. OMI is in the App MVP as bounded planning; raw idea/candidate creation, owner decision/destination updates, and record-only promotion gate enforcement exist locally, but durable promotion application remains unimplemented.
