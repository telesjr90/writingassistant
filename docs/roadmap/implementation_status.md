...
- Active child task: `PHASE8-IMPL-008-T003` ready/active (extraction artifact storage path and manifest contract tests); `PHASE8-IMPL-008-T001` and `PHASE8-IMPL-008-T002` complete.
- Next child task: `PHASE8-IMPL-008-T003` - Extraction artifact storage path and manifest contract tests.
- Last completed child task: `PHASE8-IMPL-007-T007` - Roadmap/status closeout.
...
`PHASE8-IMPL-008` is active as the eighth Writer Assistant Core parent. `PHASE8-IMPL-008-T001` published the parent task record at `docs/roadmap/tasks/PHASE8-IMPL-008.md`, the inventory at `docs/roadmap/inventory/PHASE8-IMPL-008.md`, the enrichment JSON at `docs/roadmap/enrichment/PHASE8-IMPL-008.enrichment.json`, and the roadmap/status updates. T001 confirmed the four `.external_sources/` clone paths exist locally and remain protected from commit via `.git/info/exclude`, and confirmed `backend/story_knowledge/booknlp_adapter_contract.py` and `tests/test_writer_assistant_core_booknlp_adapter_contract.py` are unchanged. `PHASE8-IMPL-008-T002` completed the raw extraction artifact storage contract decision at `docs/roadmap/decisions/PHASE8-IMPL-008-raw-extraction-artifact-storage-contract-decision.md`. T002 accepted the future storage root `project_dir / "writer_assistant" / "extractions"`, run-folder shape `writer_assistant/extractions/{tool_name}/{run_id}/`, first tool folder `booknlp`, strict tool/run ID rules, manifest shape and policy flags, raw artifact refs through existing source/evidence boundaries where compatible, `raw/` BookNLP artifact layout, optional raw-ref-only `book.html`, and `derived/events.json` only as app-owned derived support from `.tokens.event`, not as a real external BookNLP raw output file. `PHASE8-IMPL-008` builds on the completed `PHASE8-IMPL-007` mocked BookNLP adapter contract foundation and prepares a project-local raw extraction artifact storage boundary under `projects/{project_id}/writer_assistant/extractions/` and a mocked BookNLP-like fixture parser contract. The active child is `PHASE8-IMPL-008-T003` (extraction artifact storage path and manifest contract tests, ready/active). `PHASE8-IMPL-008-T004` through `PHASE8-IMPL-008-T007` are planned/draft. No raw extraction artifact storage helpers or files exist yet, no BookNLP fixture parser exists yet, no real BookNLP/spaCy install or execution exists, no runtime extraction, no routes, no UI, no package changes, no generated prose, no apply-promotion, and no memory/canon mutation exists. Owner review remains mandatory. T001/T002 did not implement extraction, did not create raw artifact helpers or files, did not implement a fixture parser, did not change the existing mocked adapter contract module or test, did not run BookNLP or spaCy, did not install any package, did not stage/commit/push, and did not run context tools, external tools, or source/web retrieval.
