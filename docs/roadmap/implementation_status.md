...
- Active child task: `PHASE8-IMPL-006-T001` - Publish evidence-first extraction foundation parent and scope decision.
- Next child task: `PHASE8-IMPL-006-T002` - Evidence/source-map contract decision.
- Last completed child task: `PHASE8-IMPL-005-T007` - Roadmap/status closeout.
...
`PHASE8-IMPL-006` is active as the sixth Writer Assistant Core parent. `PHASE8-IMPL-006-T001` publishes the evidence-first extraction foundation and BookNLP-ready adapter strategy with docs/status/planning artifacts only. This parent revises the narrow spaCy-first-only next-parent framing into stable source maps, source locator contracts, Evidence Ledger/provenance planning, raw tool output policy, candidate normalization contracts, candidate-only persistence boundaries, and a BookNLP-ready adapter contract. BookNLP is framed as a strong candidate for the first serious literary extractor after source maps/provenance exist; spaCy remains a possible lightweight local baseline/support option. dramatica-flow remains reference-only or future wrapped analysis-only rubric source; Subtxt remains semantic guardrail/rubric source; NCP remains future approved-context import/export target. No runtime extraction has started. BookNLP and spaCy are not installed or run. The next work is `PHASE8-IMPL-006-T002` - Evidence/source-map contract decision.
