...
- Active child task: `PHASE8-IMPL-002-T001` - Publish candidate storage/evidence validation parent and child-task plan.
- Next child task: `PHASE8-IMPL-002-T002` - Candidate storage and evidence/provenance contract decision.
...
- Next parent/child: `PHASE8-IMPL-002` is the active Writer Assistant Core parent; next child is `PHASE8-IMPL-002-T002` after T001 publication.
...
`PHASE8-IMPL-002` is active as the next Writer Assistant Core parent. It prepares candidate storage contracts and evidence/provenance validation before extraction/runtime expansion. `PHASE8-IMPL-002-T001` is docs/status/planning only and publishes the parent, inventory, enrichment JSON, child-task plan, and roadmap/status updates. `PHASE8-IMPL-002-T002` is the next child and must decide the exact candidate storage and evidence/provenance contract before tests or helpers are added. No extraction, candidate storage writes, backend routes, frontend extraction UI, model/Ollama calls, apply-promotion, OMI candidate promotion, or memory/canon mutation exists yet.
