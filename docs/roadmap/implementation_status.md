...
- Immediate active parent task: `PHASE8-IMPL-006` - Writer Assistant Core evidence-first extraction foundation and BookNLP-ready adapter strategy.
- Active child task: `PHASE8-IMPL-006-T007` - Roadmap/status closeout.
- Next child task: none under `PHASE8-IMPL-006`.
- Last completed child task: `PHASE8-IMPL-006-T006` - BookNLP-ready adapter contract tests.
- Last completed parent task: `PHASE8-IMPL-005` - Writer Assistant Core tool evaluation and extraction strategy decision.
...
`PHASE8-IMPL-006` is active as the sixth Writer Assistant Core parent. `PHASE8-IMPL-006-T001` published the evidence-first extraction foundation and BookNLP-ready adapter strategy with docs/status/planning artifacts only. `PHASE8-IMPL-006-T002` accepted the evidence/source-map contract decision at `docs/roadmap/decisions/PHASE8-IMPL-006-evidence-source-map-contract-decision.md`. The accepted contract defines app-owned source document identity, source maps, source locators, primary character offsets over exact UTF-8 decoded source snapshots, source hashes/snapshots, Evidence Ledger shape, evidence records, extraction run provenance, raw output policy, BookNLP-ready mapping assumptions, candidate normalization boundaries, and owner-review/no-canon guardrails before runtime extraction. `PHASE8-IMPL-006-T003` added tests-first source/evidence contract coverage in `tests/test_writer_assistant_core_source_evidence_contract.py` for future `backend.story_knowledge.source_map` and `backend.story_knowledge.evidence` helpers. `PHASE8-IMPL-006-T004` added pure validation helpers in `backend/story_knowledge/source_map.py` and `backend/story_knowledge/evidence.py`; the source/evidence contract tests now pass. `PHASE8-IMPL-006-T005` accepted the BookNLP-ready raw output and adapter contract decision at `docs/roadmap/decisions/PHASE8-IMPL-006-booknlp-ready-raw-output-adapter-contract-decision.md`. The decision defines future BookNLP-like raw artifact kinds, raw output storage boundaries, run manifest shape, raw output reference policy, mocked fixture shapes, app-owned adapter normalization boundaries, candidate/evidence mapping expectations, fail-closed behavior, and the tests-first T006 handoff. `PHASE8-IMPL-006-T006` added tests-first BookNLP-ready adapter contract coverage in `tests/test_writer_assistant_core_booknlp_adapter_contract.py` for future `backend.story_knowledge.booknlp_adapter_contract` APIs: `validate_booknlp_run_manifest`, `validate_booknlp_raw_artifact_bundle`, `normalize_booknlp_entity_mentions`, `normalize_booknlp_quotes`, `normalize_booknlp_events`, and `build_booknlp_candidate_drafts`. The targeted T006 pytest is expected red until the future module exists; the current failure is limited to missing `booknlp_adapter_contract`. This parent revises the narrow spaCy-first-only next-parent framing into stable source maps, source locator contracts, Evidence Ledger/provenance planning, raw tool output policy, candidate normalization contracts, candidate-only persistence boundaries, and a BookNLP-ready adapter contract. BookNLP is framed as a strong candidate for the first serious literary extractor after source maps/provenance exist; spaCy remains a possible lightweight local baseline/support option. dramatica-flow remains reference-only or future wrapped analysis-only rubric source; Subtxt remains semantic guardrail/rubric source; NCP remains future approved-context import/export target. No runtime extraction exists yet. BookNLP and spaCy are not installed or run. No production adapter code exists yet.
...
The published Phase 7 parent sequence (`PHASE7-IMPL-001` through `PHASE7-IMPL-010`) is complete. `PHASE8-IMPL-001`, `PHASE8-IMPL-002`, `PHASE8-IMPL-003`, `PHASE8-IMPL-004`, and `PHASE8-IMPL-005` are complete as the first five published Writer Assistant Core parents in `docs/roadmap/roadmap_index.yaml`. `PHASE8-IMPL-006` is active as the evidence-first extraction foundation and BookNLP-ready adapter strategy parent.
