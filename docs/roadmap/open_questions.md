...
41. Candidate persistence timing remains deferred beyond T005; a future owner-approved task must define project-local candidate storage usage, schema/record validation, index update behavior, source locator/evidence/provenance requirements, candidate-only status, and no memory/canon write.
42. Candidate review UI/API timing remains deferred beyond T005; a future parent must define suggestion display, evidence/provenance display, insufficient-evidence/rejected-output states, explicit owner action, and separation from apply-promotion.
43. Owner decision storage remains deferred beyond T005; future owner decisions may update candidate state only through a review mechanism and must not imply memory/canon mutation.
44. Apply-promotion remains deferred beyond T005 and must be separately owner-approved, audited, explicit, and tested before any memory/canon write path exists.
45. Raw artifact persistence remains deferred beyond T005; future raw persistence must be explicit, project-local, manifest-backed, non-canon, non-candidate, and not training data.
46. Real BookNLP/spaCy runtime extraction remains deferred beyond T005; future runtime extraction requires package/dependency, security/privacy, local execution, source-to-output mapping, and owner approval decisions.
47. Recommended next parent after PHASE8-IMPL-010 closeout is `PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning`, recommendation-only until separately published. Candidate persistence timing, review UI/API boundary, owner decision storage, apply-promotion, raw persistence, runtime extraction, and byte-to-character source matching remain deferred.
