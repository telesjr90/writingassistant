...
- Status: SIXTEENTH PARENT ACTIVE (MVP-REQUIRED). `PHASE8-IMPL-001` through `PHASE8-IMPL-015` are complete. `PHASE8-IMPL-015 - Review API route implementation and read-only frontend review queue surface` is complete through `PHASE8-IMPL-015-T007`; it delivered read-only review queue routes in `backend/routes/review_queue.py`, route contract tests, frontend read-only surface planning decision, and validation-only read-only safety regression with no hardening required, without owner action execution, apply-promotion, approved memory/canon mutation, raw artifact persistence, runtime extraction, model-assisted extraction, real BookNLP/spaCy install/run/import, NCP/Subtxt/dramatica-flow runtime integration, frontend production review surface, or generated prose/prose-production paths. `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary` is active after T006; `PHASE8-IMPL-016-T001` is complete/PASS, `PHASE8-IMPL-016-T002` is complete/PASS as the owner-action review command boundary decision, `PHASE8-IMPL-016-T003` is complete/PASS as tests-first review command owner-action candidate review route contract coverage, `PHASE8-IMPL-016-T004` is complete/PASS as the minimal backend command route/helper implementation, `PHASE8-IMPL-016-T005` is complete/PASS as the frontend owner-action review workflow/surface, and `PHASE8-IMPL-016-T006` is complete/PASS as focused owner-action workflow safety regression coverage. `PHASE8-IMPL-016-T007` is ready/active next. T006 proves `POST /api/projects/{project_id}/review-queue/{queue_entry_id}/actions` remains bounded to response-only review workflow state, forbidden commands and payload fields fail closed, frontend controls expose only allowed review workflow commands with explicit owner confirmation, candidate-only/no-canon and evidence/provenance/source locator warnings remain present, and the targeted source scan distinguishes rejected fixtures from implementation behavior without the earlier contradictory forbidden-substring issue. `PHASE8-IMPL-017` through `PHASE8-IMPL-022` remain future MVP-required parents for apply-promotion plus approved memory/canon mutation, raw artifact persistence, real BookNLP/spaCy install/run/import plus runtime extraction, model-assisted evidence-backed extraction, NCP/Subtxt/dramatica-flow analysis-only runtime integration, and end-to-end MVP usability validation. Fine-tuning remains outside the MVP critical path. Generated prose/prose-production paths remain permanently forbidden.
