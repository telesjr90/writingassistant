...
- Status: SEVENTEENTH PARENT ACTIVE (MVP-REQUIRED). `PHASE8-IMPL-001` through `PHASE8-IMPL-016` are complete. `PHASE8-IMPL-016 - Frontend owner-action execution workflow and review command boundary` is complete through `PHASE8-IMPL-016-T007`; it delivered the backend review-action command route/helper for `POST /api/projects/{project_id}/review-queue/{queue_entry_id}/actions`, the frontend owner-action review workflow/surface, and focused safety regression coverage at `tests/test_owner_action_review_workflow_safety_regression.py`, without apply-promotion, approved memory/canon mutation, raw artifact persistence, runtime extraction, model-assisted extraction, real BookNLP/spaCy install/run/import, NCP/Subtxt/dramatica-flow runtime integration, or generated prose/prose-production paths. `PHASE8-IMPL-017 - Apply-promotion contract, audit log, and approved memory/canon mutation boundary` is active after `PHASE8-IMPL-017-T006` complete/PASS. T002 accepted `docs/roadmap/decisions/PHASE8-IMPL-017-apply-promotion-boundary-audit-model-decision.md` as docs/decision/status only. T003 added contract tests at `tests/test_writer_assistant_core_apply_promotion_contract.py`. T004 implemented `backend/story_knowledge/apply_promotion.py`, `backend/routes/apply_promotion.py`, and route inclusion in `backend/main.py`; the apply-promotion contract is now green. T005 implemented the frontend API helper and confirmation surface at `frontend/src/api.js` and `frontend/src/components/ApplyPromotionConfirmation.jsx`. T006 added approved memory/canon mutation safety regression coverage at `tests/test_apply_promotion_memory_canon_safety_regression.py` plus minimal backend source marker hardening. `PHASE8-IMPL-017-T007` is ready/active next for parent closeout. `PHASE8-IMPL-017` owns explicit owner-confirmed apply-promotion, audited promotion records, and approved memory/canon mutation only after explicit owner action. It must not automatically promote from confidence, queue status, model output, extraction, candidate persistence, or raw artifact presence. Candidate persistence is not canon; queue presence is not approval; confidence is not truth; raw artifacts are support data. `PHASE8-IMPL-018` through `PHASE8-IMPL-022` remain future MVP-required parents for raw artifact persistence, real BookNLP/spaCy install/run/import plus runtime extraction, model-assisted evidence-backed extraction, NCP/Subtxt/dramatica-flow analysis-only runtime integration, and end-to-end MVP usability validation. Fine-tuning remains outside the MVP critical path. Generated prose/prose-production paths remain permanently forbidden.
