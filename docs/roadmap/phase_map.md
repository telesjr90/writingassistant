...
- Status: THIRTEENTH PARENT ACTIVE (PLANNING/CONTRACT). `PHASE8-IMPL-001` through `PHASE8-IMPL-012` are complete and `PHASE8-IMPL-013` is active after `PHASE8-IMPL-013-T002` (docs/decision only). `PHASE8-IMPL-012` is complete through `PHASE8-IMPL-012-T007` (docs/status closeout only); it delivered review queue storage validation, a project-local queue storage helper `backend/story_knowledge/review_queue_storage.py`, a derived/rebuildable index, and owner action record validation only, with the target review queue storage contract passing (359 tests), candidate review gate/candidate/orchestrator/source-evidence/parser-storage-adapter/OMI regressions passing, and the BookNLP/spaCy availability guard reporting both false. `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` is a planning/contract parent that decides whether and how the project-local review queue storage and owner action record validation should be exposed through a read-only review queue API, an owner-action command API, and a frontend review UI, without implementing any backend routes, frontend screens, owner action execution, apply-promotion, or memory/canon mutation; `PHASE8-IMPL-013-T001` published the parent task record, inventory, and enrichment JSON, and `PHASE8-IMPL-013-T002` accepted `docs/roadmap/decisions/PHASE8-IMPL-013-read-only-review-queue-api-contract-decision.md` (define the read-only review queue API contract before frontend UI but implement no routes in T002; future operations are read-only GET/read-model only and review-workflow-only and never mutate queue entries, candidate records, memory, canon, project source files, raw artifacts, or indexes; safe request/query parameters, read-only response shape, mandatory evidence/provenance/uncertainty display, filtering/sorting/pagination as workflow convenience only, fail-closed error/quarantine with no repair-by-writing, owner action API separation to T003, apply-promotion/memory-canon/runtime/raw boundaries, review UI relationship to T004, and security/path safety) and marked `PHASE8-IMPL-013-T003` (owner action command API contract decision) ready/active, with T004-T007 planned. The review queue remains workflow support only and queue presence is non-approval; owner action is not apply-promotion; owner review remains mandatory before approved truth. `PHASE8-IMPL-007` delivered the mocked BookNLP adapter contract implementation and normalization foundation. `PHASE8-IMPL-008` is complete as the raw extraction artifact storage and BookNLP fixture parser contract parent. `PHASE8-IMPL-009` is complete as the BookNLP fixture parser helper implementation and raw artifact bundle integration parent. `PHASE8-IMPL-010` is complete as the extraction orchestration planning/contract boundary parent, not real extraction. The orchestrator contract passes with 67 tests, and the fixture orchestrator remains pure, in-memory, fixture-only, candidate-draft-only, non-persistent, non-canon, non-prose, and non-runtime-extraction. `PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning` is complete after `PHASE8-IMPL-011-T007`. T001 published the parent. T002 completed `docs/roadmap/decisions/PHASE8-IMPL-011-candidate-draft-to-record-persistence-gate-decision.md`, accepting a future explicit persistence gate between in-memory `candidate_drafts` and persisted candidate records. T003 completed `docs/roadmap/decisions/PHASE8-IMPL-011-review-queue-data-shape-lifecycle-decision.md`, accepting future queue entries as workflow support only, queue presence as non-approval, lifecycle states as review workflow states only, evidence/provenance/source locator display as mandatory, insufficiency/rejected-output handling as explicit states or quarantine paths, grouping/sorting/filtering as workflow convenience only, and owner action labels as future planning terms only. T004 added expected-red contract coverage at `tests/test_writer_assistant_core_candidate_review_gate_contract.py` for the future `backend.story_knowledge.candidate_review_gate` module and its `validate_candidate_draft_for_persistence`, `build_candidate_record_from_draft`, `build_review_queue_entry`, and `persist_candidate_record_for_review` APIs; the target pytest is expected-red with a collection `ImportError` limited to the missing future module. T005 is complete and created `backend/story_knowledge/candidate_review_gate.py`, a pure, standard-library-only, deterministic, candidate-only/review-pending persistence gate over existing candidate validation/persistence helpers implementing the four contract APIs; the target contract now passes (154 tests). `PHASE8-IMPL-011-T006` is complete as a validation-only candidate review gate safety regression: the target gate contract passes (154 tests), candidate regressions pass (307 tests), orchestrator/source-evidence contracts pass (171 tests), parser/storage/adapter regressions pass (397 tests), and focused OMI/project regressions pass (109 tests); no safety gap was found, so no runtime hardening patch was needed and `candidate_review_gate.py` was left unchanged. `PHASE8-IMPL-011-T007` is complete as docs/status closeout only; `PHASE8-IMPL-011` is complete and no active child remains. T007 recorded the final parent result, final artifacts, final candidate review gate APIs, final runtime/test behavior, deferred work, tracked-artifact confirmation, and validation results, and recommended `PHASE8-IMPL-012 - Writer Assistant Core review queue storage and owner-review workflow planning` as the next parent. `PHASE8-IMPL-012` is complete through `PHASE8-IMPL-012-T007`. Its purpose was review queue storage and owner-review workflow planning, not implementation: it decided whether and how the in-memory review queue entries produced by the completed `PHASE8-IMPL-011` candidate review gate should be stored, listed, loaded, and prepared for future owner-review workflows without turning queue state into approval, canon truth, apply-promotion, review UI/API scope creep, or memory/canon mutation. T001 published the parent task record, inventory, and enrichment JSON as docs/status/planning only. `PHASE8-IMPL-012-T002` accepted `docs/roadmap/decisions/PHASE8-IMPL-012-review-queue-storage-contract-decision.md` as docs/decision only, recording a project-local, candidate-linked, candidate-only, review-workflow-only, evidence/provenance-backed, fail-closed review queue storage contract with future storage root `projects/{project_id}/writer_assistant/review_queue/`, a stored queue entry shape and allowed `review_status`/`lifecycle_state` values aligned with the `PHASE8-IMPL-011-T003` queue shape, candidate linkage/integrity rules, a derived/rebuildable index contract, a storage operation boundary, and forbidden destinations. `PHASE8-IMPL-012-T003` accepted `docs/roadmap/decisions/PHASE8-IMPL-012-owner-action-workflow-boundary-decision.md` as docs/decision only, recording owner actions as review workflow commands and states only (not apply-promotion, not memory/canon mutation, and not approval/canon truth), the allowed owner action commands and states, a future owner action record shape, the queue entry mutation boundary, the candidate record relationship, apply-promotion and memory/canon boundaries, evidence/provenance requirements, an owner action storage boundary, a deferred review UI/API boundary, and a fail-closed/quarantine policy; it marked `PHASE8-IMPL-012-T004` (review queue storage contract tests) ready/active. `PHASE8-IMPL-012-T004` added the expected-red review queue storage contract tests; `PHASE8-IMPL-012-T005` created `backend/story_knowledge/review_queue_storage.py` so the target review queue storage contract passes (359 tests); `PHASE8-IMPL-012-T006` validated review queue storage safety as a validation-only pass and required no runtime hardening patch; and `PHASE8-IMPL-012-T007` closed the parent and recommended `PHASE8-IMPL-013 - Writer Assistant Core review UI/API planning and owner-action workflow contract` (recommendation-only, not active until separately published). Review queue entries remain workflow support only and queue presence is non-approval; owner review remains mandatory before anything can become approved truth. No candidate review UI/API, owner action workflow execution, owner action storage, automatic orchestrator persistence, backend routes, frontend UI, apply-promotion, memory/canon mutation, candidate/canon/memory mutation, raw artifact write/read/list helpers, real BookNLP/spaCy install or execution, runtime extraction, package/model/prose/promotion/memory/canon work, or training/JSONL/dataset work is authorized or exists; `PHASE8-IMPL-012-T005` added only the minimal pure review queue storage helper and `PHASE8-IMPL-012-T006`/`PHASE8-IMPL-012-T007` changed no runtime code.
