...
- Status: FIFTH PARENT ACTIVE. `PHASE8-IMPL-001` is complete through `PHASE8-IMPL-001-T007`: it published the Writer Assistant Core parent and child-task sequence, planned context collection without context tools, produced the targeted context report through direct inspection only, selected Writer Assistant Core candidate schema constants plus source-level/contract tests as the first runtime slice, added tests-first contract coverage, and implemented minimal constants-only `backend.story_knowledge.candidate_schema` metadata. `PHASE8-IMPL-002` is complete through `PHASE8-IMPL-002-T007`: it accepted the candidate storage/evidence/provenance contract decision, added tests-first record and storage path contract coverage, implemented pure candidate record validation helpers in `backend/story_knowledge/candidate_record.py`, and implemented pure storage path helpers in `backend/story_knowledge/candidate_storage.py` with no file I/O or storage writes. Final `PHASE8-IMPL-002` behavior: candidate record validation helpers are pure; storage helpers are pure path helpers only; no directories/files are created; no JSON read/write/list behavior exists. `PHASE8-IMPL-003` is complete through `PHASE8-IMPL-003-T007`: it accepted the candidate persistence contract decision, added tests-first write/read and list contract coverage, implemented `write_candidate_record`, `read_candidate_record`, and `list_candidate_records` in `backend/story_knowledge/candidate_persistence.py`, and closed with final validation. Final `PHASE8-IMPL-003` behavior: candidate JSON files under `writer_assistant/candidates/*.json` are source of truth; write validates before persistence; read/list validate loaded records and are side-effect free; list is direct-file-only, non-recursive, and deterministic by `candidate_id`; no index is read/written/created. `PHASE8-IMPL-004` is complete through `PHASE8-IMPL-004-T007`: it published the derived candidate index parent and child-task plan; T002 accepted the derived index contract at `docs/roadmap/decisions/PHASE8-IMPL-004-candidate-index-contract-decision.md`; T003 added index contract tests; T004 implemented `build_candidate_index`, `write_candidate_index`, and `read_candidate_index` in `backend/story_knowledge/candidate_index.py`; T005 added focused index safety regression tests with no repair gap; T006 completed as validation-only with no repair or hardening patch required; T007 closed parent with final validation. Final `PHASE8-IMPL-004` behavior: candidate JSON files remain source of truth; `writer_assistant/index.json` is derived convenience metadata only; build derives from `list_candidate_records` and is side-effect free; write refreshes index from candidate JSON; read validates index shape only and does not rebuild/repair; stale-but-valid index can be read as-is; corrupt index raises `ValueError` on read and is ignored by build; corrupt index may be overwritten by write if candidate JSON records validate. `PHASE8-IMPL-005` is active through `PHASE8-IMPL-005-T006`: it published the tool evaluation and extraction strategy parent for exactly nine approved tools/references (dramatica-flow, Narrative Context Protocol, Subtxt docs, spaCy, segram, BookNLP, GLiNER, LangExtract, Renard); T001 recorded official source retrieval policy with T003 as first allowed retrieval child; T002 accepted evaluation scope, fixture category plan, 0-5 scoring rubric, provisional tool grouping, and T003 source inventory requirements at `docs/roadmap/decisions/PHASE8-IMPL-005-evaluation-scope-fixture-rubric-decision.md`; T003 completed official source inventory and license/dependency screen at `docs/roadmap/inventory/PHASE8-IMPL-005-tool-source-inventory.md` from official docs/repos only with no install/clone/execution; T004 accepted dramatica-flow as `reference-only` at `docs/roadmap/decisions/PHASE8-IMPL-005-dramatica-flow-analysis-only-reference-decision.md` with runtime adapter REJECT/DEFER and rejected generation/revision/continuation/world_state behaviors; T005 accepted NCP as `reference-only`/future approved-context import-export candidate and Subtxt docs as `reference-only`/future semantic rubric candidate at `docs/roadmap/decisions/PHASE8-IMPL-005-ncp-subtxt-structural-interpretation-strategy-decision.md` with runtime adapter/import/export/Subtxt analysis REJECT/DEFER; T006 accepted spaCy-first local deterministic/rule-assisted candidate extraction foundation at `docs/roadmap/decisions/PHASE8-IMPL-005-nlp-extraction-adapter-strategy-decision.md`; active child is T007; no extraction implementation has started; runtime extraction, backend routes, frontend UI, model calls, tool install/clone/execution, apply-promotion, memory/canon mutation, automatic/watcher/route/UI/extraction-triggered index refresh, and graph/vector/search index behavior remain future work.
