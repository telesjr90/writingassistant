...
- Status: SEVENTH PARENT ACTIVE. `PHASE8-IMPL-001` through `PHASE8-IMPL-006` are complete. `PHASE8-IMPL-006` closed the evidence-first extraction foundation and BookNLP-ready adapter strategy before extraction implementation. T002 accepted the evidence/source-map contract at `docs/roadmap/decisions/PHASE8-IMPL-006-evidence-source-map-contract-decision.md`, defining project-local source document identity, source maps, locators, primary character-offset evidence spans, source hashes/snapshots, Evidence Ledger/provenance, raw output policy, BookNLP-ready mapping assumptions, and candidate normalization boundaries. T003 added tests-first contract coverage in `tests/test_writer_assistant_core_source_evidence_contract.py`. T004 added pure validation helpers in `backend/story_knowledge/source_map.py` and `backend/story_knowledge/evidence.py`; source/evidence contract tests now pass. T005 accepted the BookNLP-ready raw output and adapter contract at `docs/roadmap/decisions/PHASE8-IMPL-006-booknlp-ready-raw-output-adapter-contract-decision.md`, defining BookNLP-like raw artifacts, future raw storage/run manifest boundaries, mocked fixture shapes, adapter normalization, candidate/evidence mapping expectations, and fail-closed behavior. T006 added tests-first BookNLP-ready adapter contract coverage in `tests/test_writer_assistant_core_booknlp_adapter_contract.py` for future `backend.story_knowledge.booknlp_adapter_contract` APIs; T007 closed the parent and recommended `PHASE8-IMPL-007 - Writer Assistant Core BookNLP adapter contract implementation and mocked normalization foundation` as the next parent. `PHASE8-IMPL-007` is now active as the seventh parent. T001 published the parent, inventory, enrichment JSON, and roadmap/status updates; T001 confirmed the four `.external_sources/` clones exist and are protected from commit via `.git/info/exclude`; T002 completed read-only official source inventory at `docs/roadmap/inventory/PHASE8-IMPL-007-official-source-inventory.md` and implementation refresh decision at `docs/roadmap/decisions/PHASE8-IMPL-007-implementation-source-refresh-decision.md`. T002 verified local repo SHAs, confirmed real BookNLP output field names and event derivation implications, confirmed dramatica-flow remains blocked/deferred, and confirmed NCP/Subtxt remain reference-only. T003 completed the implementation decision at `docs/roadmap/decisions/PHASE8-IMPL-007-booknlp-adapter-implementation-decision.md`, accepting T004 validators/API symbols first, T005 mocked normalizers, T006 builder/boundary hardening, raw-vs-normalized fixture separation, event derivation from `.tokens.event`, byte-offset/source-locator policy, `g` identity guardrail, and coreference/quote uncertainty guardrails. T004 created pure mocked `backend/story_knowledge/booknlp_adapter_contract.py`, exposed all six public APIs, implemented manifest and raw artifact bundle validators, supplied minimal in-memory draft shaping required by the current contract tests, and made `tests/test_writer_assistant_core_booknlp_adapter_contract.py` pass. T005 completed as validation-only after reviewing mocked entity/quote/event normalization and confirming no runtime/test repair gap. T006 completed as validation-only after reviewing the candidate draft builder, fail-closed behavior, and boundary hardening scope and confirming no runtime/test repair gap. The active next child is `PHASE8-IMPL-007-T007` for roadmap/status closeout. No real BookNLP/spaCy install or execution; no runtime extraction; no routes; no UI; no package changes; no generated prose; no apply-promotion; no memory/canon mutation. No extraction implementation has started; runtime extraction, backend routes, frontend UI, model calls, BookNLP/spaCy install or execution, apply-promotion, memory/canon mutation, automatic/watcher/route/UI/extraction-triggered index refresh, and graph/vector/search index behavior remain future work.
...
owner-authored scene/chapter/note text
  ...
  -> BookNLP-ready adapter contract implementation with mocked normalization after owner-approved PHASE8-IMPL-007 publication
