...
- Status: EIGHTH PARENT ACTIVE. `PHASE8-IMPL-001` through `PHASE8-IMPL-007` are complete. `PHASE8-IMPL-007` delivered the mocked BookNLP adapter contract implementation and normalization foundation: `backend/story_knowledge/booknlp_adapter_contract.py` exposes the six public APIs (`validate_booknlp_run_manifest`, `validate_booknlp_raw_artifact_bundle`, `normalize_booknlp_entity_mentions`, `normalize_booknlp_quotes`, `normalize_booknlp_events`, `build_booknlp_candidate_drafts`); validates pure mocked manifests and raw artifact bundles; normalizes mocked entity/quote/event records; builds pure mocked in-memory candidate drafts; fails closed on invalid, unsupported, ambiguous, or insufficiently evidenced input; and preserves source/evidence/provenance/raw-ref integration. `PHASE8-IMPL-008` is active as the raw extraction artifact storage and BookNLP fixture parser contract parent: it prepares a project-local raw extraction artifact storage boundary under `projects/{project_id}/writer_assistant/extractions/` and a mocked BookNLP-like fixture parser contract without real BookNLP/spaCy install or execution, without runtime extraction, without routes/UI/package changes, and without apply-promotion or memory/canon mutation. `PHASE8-IMPL-008-T001`, `T002`, `T003`, `T004`, and `T005` are complete; `T005` accepted the BookNLP fixture parser contract decision at `docs/roadmap/decisions/PHASE8-IMPL-008-booknlp-fixture-parser-contract-decision.md`, recording the in-memory only fixture parser contract, future module `backend/story_knowledge/booknlp_fixture_parser.py`, future public APIs, exact TSV header validation, numeric coercion, `.book` JSON parsing with `g` as raw aggregate metadata only, event derivation from `.tokens.event` only, raw bundle builder compatibility with `validate_booknlp_raw_artifact_bundle`, T006 tests-first expected-red, and parser implementation deferred to `PHASE8-IMPL-009`. `PHASE8-IMPL-008-T006` is ready/active; `PHASE8-IMPL-008-T007` is planned/draft. Recommended Phase 8 next parent, not active until separately published: `PHASE8-IMPL-009 - Writer Assistant Core BookNLP fixture parser helper implementation and raw artifact bundle integration`.
