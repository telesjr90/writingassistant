...
- Status: TENTH PARENT ACTIVE. `PHASE8-IMPL-001` through `PHASE8-IMPL-009` are complete, and `PHASE8-IMPL-010` is active. `PHASE8-IMPL-007` delivered the mocked BookNLP adapter contract implementation and normalization foundation: `backend/story_knowledge/booknlp_adapter_contract.py` exposes the six public APIs (`validate_booknlp_run_manifest`, `validate_booknlp_raw_artifact_bundle`, `normalize_booknlp_entity_mentions`, `normalize_booknlp_quotes`, `normalize_booknlp_events`, `build_booknlp_candidate_drafts`); validates pure mocked manifests and raw artifact bundles; normalizes mocked entity/quote/event records; builds pure mocked in-memory candidate drafts; fails closed on invalid, unsupported, ambiguous, or insufficiently evidenced input; and preserves source/evidence/provenance/raw-ref integration. `PHASE8-IMPL-008` is complete as the raw extraction artifact storage and BookNLP fixture parser contract parent: it prepares a project-local raw extraction artifact storage boundary under `projects/{project_id}/writer_assistant/extractions/` and a mocked BookNLP-like fixture parser contract without real BookNLP/spaCy install or execution, without runtime extraction, without routes/UI/package changes, and without apply-promotion or memory/canon mutation. `PHASE8-IMPL-009` is complete as the BookNLP fixture parser helper implementation and raw artifact bundle integration parent. `PHASE8-IMPL-009-T001` through `PHASE8-IMPL-009-T007` are complete. T003 created `backend/story_knowledge/booknlp_fixture_parser.py` and implemented pure in-memory TSV parsers for tokens, entities, quotes, and supersense. T004 implemented pure in-memory `.book` JSON parsing and token-event derivation from token `event` markers, keeping `.book.g` raw and derived events support-only. T005 implemented pure in-memory raw artifact bundle builder integration with storage/source/evidence validation and adapter bundle compatibility. T006 validated the full parser contract, source-level boundary scan, and focused regressions with no parser code changes needed. T007 closed the parent. Parser contract collection passes and the full parser contract passes. `PHASE8-IMPL-010` is active as the extraction orchestration planning/contract boundary parent, not real extraction. `PHASE8-IMPL-010-T001` published the parent, `PHASE8-IMPL-010-T002` is complete as of 2026-06-23 with the review-safe extraction pipeline contract decision, `PHASE8-IMPL-010-T003` is complete as of 2026-06-24 with expected-red extraction orchestration contract tests in `tests/test_writer_assistant_core_extraction_orchestrator_contract.py`, `PHASE8-IMPL-010-T004` is complete as of 2026-06-24 with `backend/story_knowledge/extraction_orchestrator.py` implementing `validate_extraction_pipeline_request`, `build_fixture_extraction_pipeline_plan`, and `run_fixture_extraction_pipeline`, `PHASE8-IMPL-010-T005` is complete as of 2026-06-24 with the candidate review handoff and persistence boundary decision, and `PHASE8-IMPL-010-T006` is complete as of 2026-06-24 as a validation-only orchestration safety regression pass with no runtime hardening patch needed. The orchestrator contract passes with 67 tests, and T006 validated parser, raw storage, adapter, source/evidence, candidate, and focused OMI/project regressions. `PHASE8-IMPL-010-T007` is complete as of 2026-06-24 and closes the parent. `PHASE8-IMPL-010` is complete. The recommended next parent is `PHASE8-IMPL-011 - Writer Assistant Core candidate review queue and persistence gate planning`, recommendation-only until separately published. No raw artifact write/read/list helpers exist, no real BookNLP/spaCy install or execution exists, no runtime extraction exists, and no routes/UI/package/model/prose/promotion/memory/canon work is authorized.
