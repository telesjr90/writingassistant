...
- `PHASE8-IMPL-019-T004` is ready/active next for minimal guarded dependency availability and import/run probe implementation.
- `PHASE8-IMPL-019-T005` through `PHASE8-IMPL-019-T007` remain planned.
...
- T003 handoff: expected-red tests for future `backend/story_knowledge/runtime_extraction.py` APIs `validate_runtime_extraction_environment`, `check_booknlp_availability`, `check_spacy_availability`, `validate_runtime_extraction_request`, `build_runtime_extraction_plan`, `run_runtime_extraction_probe`, `run_guarded_runtime_extraction`, `build_raw_artifact_handoff`, `build_candidate_review_handoff`, and `quarantine_runtime_extraction_output`; no implementation.
...
- Status: NINETEENTH PARENT ACTIVE (MVP-REQUIRED). `PHASE8-IMPL-001` through `PHASE8-IMPL-018` are complete. `PHASE8-IMPL-019` is active after `PHASE8-IMPL-019-T003`; T001 published the parent, T002 accepted the Guarded runtime extraction boundary and environment model decision, and T003 added expected-red contract tests at `tests/test_writer_assistant_core_runtime_extraction_contract.py` without implementation. `PHASE8-IMPL-019-T004` is ready/active next for minimal guarded dependency availability and import/run probe implementation. `PHASE8-IMPL-019-T005` through `PHASE8-IMPL-019-T007` remain planned. The accepted T002 boundary requires BookNLP/spaCy dependency policy, explicit availability, environment guards, install/import/run separation, no silent fallback, owner-authored or owner-provided sources, PHASE8-IMPL-018 raw artifact persistence, candidate-first owner review, confidence is not truth, raw tool output is never authoritative, no automatic canon, no apply-promotion, no memory/canon mutation, no model calls, no generated prose, and no training artifacts. Future states include disabled, unavailable, dependency_missing, model_missing, configuration_invalid, probe_failed, runtime_failed, malformed_output, unsafe_path, missing_source_refs, missing_evidence_refs, missing_provenance_refs, missing_source_locator_refs, quarantined, rejected, and valid. T003 tests target `backend/story_knowledge/runtime_extraction.py` APIs including `validate_runtime_extraction_environment`, `check_booknlp_availability`, `check_spacy_availability`, `validate_runtime_extraction_request`, `build_runtime_extraction_plan`, `run_runtime_extraction_probe`, `run_guarded_runtime_extraction`, `build_raw_artifact_handoff`, `build_candidate_review_handoff`, and `quarantine_runtime_extraction_output`. `PHASE8-IMPL-020` through `PHASE8-IMPL-022` remain future MVP-required parents for model-assisted evidence-backed extraction, NCP/Subtxt/dramatica-flow analysis-only runtime integration, and end-to-end MVP usability validation. Fine-tuning remains outside the MVP critical path. Generated prose/prose-production paths remain permanently forbidden.
