...
- `PHASE8-IMPL-019-T007` is ready/active next for parent closeout.
...
- Status: NINETEENTH PARENT ACTIVE (MVP-REQUIRED). `PHASE8-IMPL-001` through `PHASE8-IMPL-018` are complete. `PHASE8-IMPL-019` is active after `PHASE8-IMPL-019-T006`; T001 published the parent, T002 accepted the Guarded runtime extraction boundary and environment model decision, T003 added expected-red contract tests at `tests/test_writer_assistant_core_runtime_extraction_contract.py`, T004 implemented the minimal guarded helper at `backend/story_knowledge/runtime_extraction.py`, T005 added guarded raw artifact persistence handoff through PHASE8-IMPL-018 helpers with focused contract tests, and T006 added focused safety regression coverage plus minimal helper hardening. `PHASE8-IMPL-019-T007` is ready/active next for parent closeout. The accepted T002 boundary requires BookNLP/spaCy dependency policy, explicit availability, environment guards, install/import/run separation, no silent fallback, owner-authored or owner-provided sources, PHASE8-IMPL-018 raw artifact persistence, candidate-first owner review, confidence is not truth, raw tool output is never authoritative, no automatic canon, no apply-promotion, no memory/canon mutation, no model calls, no generated prose, and no training artifacts. Future states include disabled, unavailable, dependency_missing, model_missing, configuration_invalid, probe_failed, runtime_failed, malformed_output, unsafe_path, missing_source_refs, missing_evidence_refs, missing_provenance_refs, missing_source_locator_refs, quarantined, rejected, and valid. T005 APIs include `persist_runtime_extraction_raw_artifacts` plus the T004 validation/probe/handoff APIs; T006 proves these APIs fail closed for unsafe paths, malformed outputs, forbidden artifacts, unavailable/quarantine states, and persistence failure without installing dependencies, adding routes/UI, persisting candidates/review entries, mutating canon/memory, calling models, creating training artifacts, running full runtime extraction over project text, or generating prose. `PHASE8-IMPL-020` through `PHASE8-IMPL-022` remain future MVP-required parents for model-assisted evidence-backed extraction, NCP/Subtxt/dramatica-flow analysis-only runtime integration, and end-to-end MVP usability validation. Fine-tuning remains outside the MVP critical path. Generated prose/prose-production paths remain permanently forbidden.
