...
- `PHASE8-IMPL-019-T005` is ready/active next for guarded runtime extraction request and raw artifact handoff implementation.
...
- T004 implementation: minimal guarded helper APIs `validate_runtime_extraction_environment`, `check_booknlp_availability`, `check_spacy_availability`, `validate_runtime_extraction_request`, `build_runtime_extraction_plan`, `run_runtime_extraction_probe`, `run_guarded_runtime_extraction`, `build_raw_artifact_handoff`, `build_candidate_review_handoff`, and `quarantine_runtime_extraction_output`; disabled/fail-closed by default; no dependency install, package edit, route, UI, model call, canon mutation, candidate/review persistence, training artifact, full runtime extraction over project text, or generated prose.
...
- Status: NINETEENTH PARENT ACTIVE (MVP-REQUIRED). `PHASE8-IMPL-001` through `PHASE8-IMPL-018` are complete. `PHASE8-IMPL-019` is active after `PHASE8-IMPL-019-T004`; T001 published the parent, T002 accepted the Guarded runtime extraction boundary and environment model decision, T003 added expected-red contract tests at `tests/test_writer_assistant_core_runtime_extraction_contract.py`, and T004 implemented the minimal guarded helper at `backend/story_knowledge/runtime_extraction.py`. `PHASE8-IMPL-019-T005` is ready/active next for guarded runtime extraction request and raw artifact handoff implementation. `PHASE8-IMPL-019-T006` and `PHASE8-IMPL-019-T007` remain planned. The accepted T002 boundary requires BookNLP/spaCy dependency policy, explicit availability, environment guards, install/import/run separation, no silent fallback, owner-authored or owner-provided sources, PHASE8-IMPL-018 raw artifact persistence, candidate-first owner review, confidence is not truth, raw tool output is never authoritative, no automatic canon, no apply-promotion, no memory/canon mutation, no model calls, no generated prose, and no training artifacts. Future states include disabled, unavailable, dependency_missing, model_missing, configuration_invalid, probe_failed, runtime_failed, malformed_output, unsafe_path, missing_source_refs, missing_evidence_refs, missing_provenance_refs, missing_source_locator_refs, quarantined, rejected, and valid. T004 APIs include `validate_runtime_extraction_environment`, `check_booknlp_availability`, `check_spacy_availability`, `validate_runtime_extraction_request`, `build_runtime_extraction_plan`, `run_runtime_extraction_probe`, `run_guarded_runtime_extraction`, `build_raw_artifact_handoff`, `build_candidate_review_handoff`, and `quarantine_runtime_extraction_output`; T004 did not install dependencies, add routes/UI, persist candidates/review entries, mutate canon/memory, call models, create training artifacts, run full runtime extraction over project text, or generate prose. `PHASE8-IMPL-020` through `PHASE8-IMPL-022` remain future MVP-required parents for model-assisted evidence-backed extraction, NCP/Subtxt/dramatica-flow analysis-only runtime integration, and end-to-end MVP usability validation. Fine-tuning remains outside the MVP critical path. Generated prose/prose-production paths remain permanently forbidden.
