...
| WORKSPACE-025 | Project memory/canon cross-linking and health | `docs/roadmap/project_memory_canon_cross_linking_health_spec.md`; shared approved-memory record/link identity, future `memory/index.json` and category registry planning, count snapshots, local filtering, non-destructive health warnings, candidate/canon separation, no-prose/no-silent-promotion boundaries, and explicit non-Dramatica proof boundaries |
| WORKSPACE-026 | Project Workspace Foundation implementation decision sweep | `docs/roadmap/project_workspace_implementation_decision_sweep.md`; consolidates WORKSPACE-001 through WORKSPACE-025 into readiness decisions, implementation defaults, owner decisions, the recommended PHASE7-IMPL sequence, the recommended first runtime task, future safety prompt, and Phase 7 acceptance checklist |

Implementation should not start Dramatica-specific analysis, extractor dependencies, or memory/canon apply-promotion before the basic workspace create/select/save/reload flows exist. The recommended first runtime task after WORKSPACE-026 is `PHASE7-IMPL-001 - Project Creation and Safe Project Metadata Backend`.
