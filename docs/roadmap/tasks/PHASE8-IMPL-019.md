...
T005 is complete/PASS as the guarded runtime extraction request and raw artifact handoff implementation at `backend/story_knowledge/runtime_extraction.py` with focused coverage at `tests/test_writer_assistant_core_runtime_extraction_raw_artifact_handoff_contract.py`. It adds an explicit `persist_runtime_extraction_raw_artifacts` handoff that validates support-data-only runtime output, preserves source/evidence/provenance/source-locator refs, persists valid bundles through PHASE8-IMPL-018 raw artifact helpers, and fails closed or quarantines malformed/unsafe/incomplete output. T005 does not implement full BookNLP/spaCy extraction over project text, install dependencies, add routes/UI, call models/Ollama, persist candidates or review queue entries, mutate approved memory/canon, apply promotion, create training artifacts, or generate prose.

T006 is complete/PASS as focused runtime extraction safety regression coverage at `tests/test_writer_assistant_core_runtime_extraction_safety_regression.py` plus minimal hardening in `backend/story_knowledge/runtime_extraction.py`. It proves guarded runtime extraction fails closed for unsafe source paths, unsafe ids, unsupported/non-owner source claims, missing refs, invalid source locators, forbidden artifact/action types, forbidden raw artifact destinations, forbidden payload markers, malformed output, unavailable states, quarantine states, and raw artifact persistence failures. T006 preserves the PHASE8-IMPL-018 raw artifact helper as the only valid persistence location, keeps raw artifacts support data only, and confirms no model calls, generated prose, training artifacts, apply-promotion, approved memory/canon mutation, candidate persistence, or review queue writes occur.
