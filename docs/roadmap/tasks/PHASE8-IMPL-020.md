...
- `PHASE8-IMPL-020-T003` - Expected-red model-assisted extraction contract tests. ready/active. Output/scope: tests-first coverage only for future model-assisted request/output guards, evidence-backed diagnostic/candidate handoff, no automatic canon, no apply-promotion, no memory/canon mutation, no training artifacts, no generated prose, no rewrite, no continuation, no outline generation, no model output as truth, fail closed, and no silent fallback.
