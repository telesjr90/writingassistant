...
`PHASE8-UX-002-T006A` is complete/PASS as the Notes/Materials project-scoped evidence UI slice for `UX2-NOTES-MATERIALS-001` only. T006A adds frontend-only create/save/reload evidence using existing note/material routes, with `createOwnerAuthoredNote`, `createOwnerProvidedMaterial`, and `reloadProjectScopedNotesMaterials`; browser-visible controls/evidence identify owner-authored notes, owner-provided materials, not-canon-by-default status, and that notes/materials do not mutate memory or canon. Focused validation command `.venv-unsloth-clean/bin/python -m pytest tests/test_phase8_ux002_mvp_ui_acceptance_expected_red.py -q -k ux2_notes_materials_001` passes with `1 passed, 6 deselected in 0.03s`. T006 remains in progress; remaining expected-red surfaces are `UX2-RAW-ARTIFACT-001`, `UX2-REVIEW-PROMOTION-001`, and `UX2-ANALYSIS-RUNTIME-001`.

`PHASE8-UX-002-T006B` is complete/PASS as the raw artifact/read-only evidence and analysis-runtime status-label UI slice for `UX2-RAW-ARTIFACT-001` and `UX2-ANALYSIS-RUNTIME-001` only. T006B adds frontend-only raw artifact evidence status through `fetchRawArtifactEvidenceStatus`, browser-visible runtime extraction unavailable/read-only raw artifact evidence labels, and `NOT_EXPOSED` analysis runtime labels for NCP, Subtxt, and dramatica-flow. Focused validation command `.venv-unsloth-clean/bin/python -m pytest tests/test_phase8_ux002_mvp_ui_acceptance_expected_red.py -q -k "ux2_raw_artifact_001 or ux2_analysis_runtime_001"` passes with `2 passed, 5 deselected in 0.03s`.

`PHASE8-UX-002-T006C` is complete/PASS as the review/apply-promotion evidence UI slice for `UX2-REVIEW-PROMOTION-001`. T006C adds browser-visible review queue fixture evidence, explicit owner confirmation labeling for apply-promotion, promotion audit evidence, and unchanged approved memory/canon evidence for failed and rejected promotion outcomes. Focused validation command `.venv-unsloth-clean/bin/python -m pytest tests/test_phase8_ux002_mvp_ui_acceptance_expected_red.py -q -k ux2_review_promotion_001` passes with `1 passed, 6 deselected in 0.03s`. The full UX2 expected-red acceptance file now passes with `.venv-unsloth-clean/bin/python -m pytest tests/test_phase8_ux002_mvp_ui_acceptance_expected_red.py -q` -> `7 passed in 0.03s`. `PHASE8-UX-002-T006` is complete/PASS.

`PHASE8-UX-002-T007` is complete/PARTIAL as closeout plus owner acceptance harness rerun. Required validation passed: roadmap/enrichment checks PASS, enrichment JSON parses, full UX2 pytest passes with `20 passed in 0.05s`, frontend build passes, and all four OMI browser evidence surfaces pass. The documented owner acceptance harness command `node scripts/mvp-owner-acceptance-browser-smoke.mjs` exited `0` and wrote evidence under `artifacts/mvp-readiness/owner-acceptance/`, but final automated decision remains `MANUAL_REVIEW_REQUIRED`. T007 records that apply-promotion confirmation is guarded evidence UI only, final Memory/Canon mutation remains separately controlled, no generated prose controls were added, no backend code changed, and MVP owner acceptance remains pending.
...
- `PHASE8-UX-002-T006` - Implement Notes/Materials + runtime/review evidence UI. Scope: split into focused slices. T006A Notes/Materials save-reload proof is complete/PASS for `UX2-NOTES-MATERIALS-001`; T006B raw artifact read-only/support-data evidence and analysis-runtime label evidence is complete/PASS for `UX2-RAW-ARTIFACT-001` and `UX2-ANALYSIS-RUNTIME-001`; T006C review/apply-promotion fixture, confirmation, audit, and unchanged-memory/canon evidence is complete/PASS for `UX2-REVIEW-PROMOTION-001`. Status: complete/PASS. Full UX2 acceptance file passes with `7 passed in 0.03s`.
- `PHASE8-UX-002-T007` - Closeout + rerun owner acceptance harness. Scope: docs/status/governance plus owner-run evidence. Re-run owner acceptance harness after missing UI surfaces are built. Do not mark MVP complete unless owner explicitly accepts through the correct checklist/roadmap gate. Status: complete/PARTIAL; owner acceptance harness exit `0`, final automated decision `MANUAL_REVIEW_REQUIRED`.
