...
- Decision record: `docs/roadmap/decisions/PHASE8-IMPL-023-T012A-real-local-runtime-tools-required-for-omi-mvp.md`.
- Scope: roadmap truth files only; no product code, tests, owner-acceptance artifacts, PDFs, or zips.
- Decision: `PHASE8-IMPL-023` cannot close as MVP-complete until OMI and analysis run real local/runtime analysis through all selected tools, or any unavailable tool is explicitly documented as BLOCKED by owner decision.
...
- Preserved truth: T004 deterministic extraction is fallback/safety baseline only; T005-T011 fixture/contracts/fusion/persistence are useful scaffolding; T012A is the live local/runtime integration reset; T013 onward are real runtime connection and validation tasks; MVP closeout is blocked until live OMI/analysis runtime validation passes or selected unavailable tools are explicitly owner-blocked.
- Corrected next sequence: T012A real local/runtime tools required for OMI MVP roadmap reset; T013 runtime configuration, preflight, health checks, and feature flags; T014 live spaCy; T015 live Ollama/local model; T016 live Story Check; T017 live BookNLP; T018 live NCP; T019 live Subtxt; T020 live dramatica-flow; T021 real-output fusion validation; T022 real-output candidate-only persistence validation; T023 grouped owner-review UI for real runtime findings; T024 automated end-to-end live OMI test; T025 manual Cyber Detective Story live OMI test; T026 closeout only after live runtime tools are connected/tested or explicitly owner-blocked.
- Safety confirmations: analysis-only, candidate-first, evidence/provenance-backed, owner-controlled, owner-authored prose storage/editing allowed, no generated prose, no rewrite/continuation/outline/draft/polish/improvement/expansion/imitation/revision/writing suggestion, confidence/support is not truth, candidate presence is not canon, candidate persistence is not canon, queue presence is not approval, tool/model output is not canon, no Memory/Canon mutation, no promotion records, no apply-promotion.
...
- Current next child task: `PHASE8-IMPL-023-T012A - Real local/runtime tools required for OMI MVP roadmap reset`.
...
- Historical next child at T010 closeout: `PHASE8-IMPL-023-T011 - Candidate-only persistence for fused AI/tool-assisted findings`; T011 is now complete/PASS and current next child is `PHASE8-IMPL-023-T012A - Real local/runtime tools required for OMI MVP roadmap reset`.
...
- Decision record: `docs/roadmap/decisions/PHASE8-IMPL-023-T008-story-check-diagnostic-only-omi-handoff.md`.
- Scope: strict `story_check` JSON/dict fixture validation and normalization only. No live Story Check runtime call; no frontend UI; no live Ollama/model, BookNLP/spaCy, NCP/Subtxt/dramatica-flow run; no Memory/Canon mutation; no apply-promotion; no story prose.
- Module updated: `backend/omi_analysis_orchestrator.py`.
- Test added: `tests/test_omi_story_check_adapter_contract.py`.
- Contract behavior: no fixture returns `unavailable`; valid `omi_story_check_diagnostic_handoff.v1` fixtures normalize to candidate-only diagnostic findings/questions; invalid JSON/schema/status/adapter/finding type/missing evidence/source locator/provenance/unsafe/no-prose/truth/final/auto-approval/review-approved/Memory-Canon-mutation/candidate-persistence/promotion/apply-promotion output fails closed with no findings and no persistence.
- Persistence: `story_check` findings are never persisted in T008, even when `persist_candidates=True`.
- Deterministic fallback status: still off by default, opt-in only, and `fallback_only`.
- Other adapters: NCP/Subtxt/dramatica-flow deferred to T009.
- Validation: `python3 -m py_compile backend/omi_analysis_orchestrator.py` -> PASS; `.venv-unsloth-clean/bin/python -m pytest tests/test_omi_story_check_adapter_contract.py -q` -> `11 passed`; `.venv-unsloth-clean/bin/python -m pytest tests/test_omi_booknlp_spacy_adapter_contract.py -q` -> `10 passed`; `.venv-unsloth-clean/bin/python -m pytest tests/test_omi_ollama_model_adapter_contract.py -q` -> `19 passed`; `.venv-unsloth-clean/bin/python -m pytest tests/test_omi_tool_assisted_orchestrator_contract.py -q` -> `26 passed`; focused OMI regressions passed: extraction contract `5 passed`, expected-red backend extraction `4 passed`, routes `20 passed`, project manager OMI subset `18 passed, 51 deselected`, boundaries `21 passed`, manual workflow source `7 passed`.
- Roadmap validation: `python3 scripts/check_enrichment.py` -> PASS; `python3 scripts/validate_roadmap.py` -> PASS; `python3 -m json.tool docs/roadmap/enrichment/PHASE8-IMPL-023.enrichment.json >/dev/null` -> PASS; `git diff --check` -> PASS.
...
- Owner override decision: `docs/roadmap/decisions/PHASE8-owner-override-block-full-mvp-closeout-omi-extraction-required.md`.
...
- Next roadmap step: `PHASE8-IMPL-023-T001`; accepted owner gate remains historical and does not complete MVP.
- No product code, tests, browser harness scripts, product UI, Memory/Canon, candidates, extraction, models/Ollama, apply-promotion, Story Check OMI decision routes, story prose, or context tools changed or ran for this decision record.
...
- Historical next roadmap step was explicit owner Accepted/Blocked gate decision. Current next step is `PHASE8-IMPL-023-T001` by owner override.
...
- `PHASE8-UX-003-B`: PASS, wired to existing OMI Dashboard, Candidate Detail, Evidence Drawer, and Apply-Promotion Confirmation PASS evidence.
- `PHASE8-UX-003-C-NON-CYBER`: PASS, wired to existing Notes/Materials project-scoped save/reload proof and NCP/Subtxt/dramatica-flow analysis-runtime label/status evidence.
- `PHASE8-UX-003-C-CYBER`: `MANUAL_REVIEW_REQUIRED`, intentionally left for `PHASE8-UX-003-T005`.
- Focused expected-red/green coverage: `.venv-unsloth-clean/bin/python -m pytest tests/test_phase8_ux003_owner_harness_expected_red.py -q` -> `6 passed in 0.05s`.
- Owner harness source/unit coverage: `.venv-unsloth-clean/bin/python -m pytest tests/test_mvp_owner_acceptance_browser_smoke_source.py -q` -> `24 passed in 0.12s`.
- Owner acceptance harness: `node scripts/mvp-owner-acceptance-browser-smoke.mjs` -> exit `0`; final automated decision `MANUAL_REVIEW_REQUIRED`.
- Evidence artifacts updated: `artifacts/mvp-readiness/owner-acceptance/evidence-report.md`, `artifacts/mvp-readiness/owner-acceptance/checklist-results.json`, and `artifacts/mvp-readiness/owner-acceptance/workflow-log.json`.
- Owner acceptance remains pending. MVP is not complete.
- No frontend/backend product code changed, no new product UI was added, no candidates were created outside authorized isolated harness behavior, Memory/Canon was not mutated, apply-promotion was not enabled or run, runtime extraction was not executed, no model-backed generation was required for T004 route wiring, and no generated prose controls were added.
...
- Expected-red command: `.venv-unsloth-clean/bin/python -m pytest tests/test_phase8_ux003_owner_harness_expected_red.py -q`.
- Expected-red result: `2 failed, 3 passed in 0.07s`.
- The two failures are intentional and limited to missing PHASE8-UX-003 owner harness coverage markers:
  - `PHASE8-UX-003-B`
  - `PHASE8-UX-003-C`
- These expected-red failures are not product failures and do not mark T004/T005 implementation complete.
- A-category blockers remain manual owner review only.
...
- Owner acceptance was not marked PASS. MVP was not marked complete. No frontend/backend/browser harness code changed, no tests were edited in this closeout, no candidates were created, Memory/Canon was not mutated, models/Ollama were not called, extraction was not run, apply-promotion was not enabled or run, and no generated prose controls were added.
