#!/usr/bin/env node
/**
 * OMI Apply-Promotion Confirmation browser-visible evidence harness.
 *
 * Evidence-only runner. It opens the OMI shell-backed Apply-Promotion
 * Confirmation, captures desktop/mobile screenshots, and proves blocked
 * confirmation cannot submit. It blocks mutating API methods and does not
 * create candidates, call models, run extraction, run apply-promotion, or
 * mutate Memory/Canon.
 */
...
const PROJECT_ID = process.env.OMI_APPLY_PROMOTION_PROJECT_ID ?? 'example';
const EVIDENCE_DIR = path.resolve(
  process.env.OMI_APPLY_PROMOTION_EVIDENCE_DIR
    ?? 'docs/roadmap/validation/omi-apply-promotion-browser-evidence',
);
const VALIDATION_MD = path.resolve(
  process.env.OMI_APPLY_PROMOTION_VALIDATION_MD
...
const FIXTURE_CANDIDATE = Object.freeze({
  candidate_id: 'fixture-apply-candidate-001',
  candidate_type: 'character_profile',
  status: 'pending_review',
  destination: '',
  destination_type: '',
  source_id: 'owner-input-apply-fixture-001',
  source_locator: 'owner-input-apply-fixture-001#L1',
  ...
  approved_memory_canon_before_state: null,
  field_review_rows: [
    {
      field: 'Name',
...
async function detectApiMode() {
  ...
  try {
    ...
    if (asArray(omi?.candidates).length === 0 && !Number(omi?.candidate_count)) {
      apiMode = 'fixture';
      logAction('live_backend_has_no_apply_candidate_using_read_only_fixture', { omiUrl });
      return;
    }
...
async function installApiGuard(context) {
  await context.route('**/api/**', async (route) => {
    ...
    if (pathname.includes('/apply-promotion')) {
      applyPromotionRequests.push({ method, pathname });
    }

    if (method !== 'GET') {
      mutatingApiRequests.push({ method, pathname });
      await route.fulfill(jsonResponse({ detail: 'Mutating API requests are blocked in OMI apply-promotion evidence.' }, 405));
      return;
    }
...
async function openApplyPromotionConfirmation() {
  ...
  const handoff = page.getByRole('button', { name: /Open handoff queue for Promotion Handoff Readiness/i }).first();
  await handoff.click();
  await page.locator('[data-testid="omi-apply-promotion-confirmation"]').waitFor({ state: 'visible', timeout: 15000 });
  await page.waitForTimeout(250);
  logAction('omi_apply_promotion_confirmation_opened');
}
...
async function assertNoGeneratedProseControls(scope, viewName) {
  const interactiveLabels = await scope
...
async function assertConfirmationEvidence(viewName) {
  const confirmation = page.locator('[data-testid="omi-apply-promotion-confirmation"]').first();
  const blockers = page.locator('[data-testid="omi-apply-promotion-blockers"]').first();
  const checkbox = confirmation.locator('input[type="checkbox"]').first();
  const finalAction = page.locator('[data-testid="omi-apply-promotion-final-action"]').first();
  const cancel = page.getByRole('button', { name: /^Cancel \/ Return$/ }).first();
  const text = await visibleText(confirmation);

  assertEvidence(`${viewName}: Apply-Promotion Confirmation is reachable`, await confirmation.isVisible(), {});
  ...
  for (const requiredText of [
    ...
    assertEvidence(`${viewName}: required confirmation copy visible: ${requiredText}`, text.includes(requiredText), {});
  }

  const blockerBox = await blockers.boundingBox();
  const checkboxBox = await checkbox.boundingBox();
  assertEvidence(`${viewName}: blockers appear before confirmation control`, Boolean(
    blockerBox && checkboxBox && blockerBox.y < checkboxBox.y,
  ), { blockerBox, checkboxBox });

  assertEvidence(`${viewName}: final action disabled when blocker visible`, await finalAction.isDisabled(), {});
  const describedBy = await finalAction.getAttribute('aria-describedby');
  const reasonIds = describedBy ? describedBy.split(/\s+/) : [];
  assertEvidence(`${viewName}: disabled final action reason associated`, reasonIds.length >= 2, { describedBy });
  for (const reasonId of reasonIds) {
    assertEvidence(`${viewName}: described reason exists ${reasonId}`, await page.locator(`#${reasonId}`).count() === 1, {
      reasonId,
    });
  }

  assertEvidence(`${viewName}: owner final confirmation visible`, await checkbox.isVisible(), {});
  assertEvidence(`${viewName}: cancel return visible`, await cancel.isVisible(), {});
  assertEvidence(`${viewName}: cancel return keyboard reachable`, await cancel.evaluate((element) => element.tabIndex >= 0), {});

  await checkbox.check();
  assertEvidence(`${viewName}: final action remains disabled after checkbox while blockers visible`, await finalAction.isDisabled(), {});
  await finalAction.click({ force: true }).catch(() => {});
  assertEvidence(`${viewName}: no apply-promotion request is made in blocked state`, applyPromotionRequests.length === 0, {
    applyPromotionRequests,
  });

  await assertNoGeneratedProseControls(confirmation, viewName);
}

async function assertNoEnabledApplyOutsideConfirmation() {
  const dashboard = page.locator('[data-testid="omi-dashboard"]').first();
  await page.getByRole('button', { name: /^Cancel \/ Return$/ }).first().click();
  await dashboard.waitFor({ state: 'visible', timeout: 15000 });
  const dashboardApply = page.getByRole('button', { name: /^Apply to Memory\/Canon$/ }).first();
  assertEvidence('Dashboard does not expose enabled apply-promotion', await dashboardApply.isDisabled(), {});
  ...
  const detailApply = page.getByRole('button', { name: /^Apply to Memory\/Canon$/ }).first();
  assertEvidence('Candidate Detail does not expose enabled apply-promotion', await detailApply.isDisabled(), {});

  const evidenceTrigger = page.locator('[data-testid="omi-evidence-drawer-trigger"]').first();
  await evidenceTrigger.click();
  const drawer = page.locator('[data-testid="omi-evidence-drawer"]').first();
  await drawer.waitFor({ state: 'visible', timeout: 15000 });
  const drawerApplyCount = await drawer.getByRole('button', { name: /Apply to Memory\/Canon/i }).count();
  assertEvidence('Evidence Drawer does not expose enabled apply-promotion', drawerApplyCount === 0, { drawerApplyCount });
}
...
async function runWorkflow() {
  ...
  await openApplyPromotionConfirmation();
  await assertConfirmationEvidence('desktop');
  desktopScreenshotPath = await screenshot('01-desktop-omi-apply-promotion-confirmation');

  afterSnapshot = await readOnlyProjectSnapshot('after');
  assertEvidence('opening confirmation does not create candidates', beforeSnapshot.omi.candidateCount === afterSnapshot.omi.candidateCount, {
    before: beforeSnapshot.omi.candidateCount,
    after: afterSnapshot.omi.candidateCount,
  });
  assertEvidence('opening confirmation does not mutate approved Memory/Canon snapshot', (
  ...
  assertEvidence('blocked confirmation performs no mutating API requests', mutatingApiRequests.length === 0, {
    mutatingApiRequests,
    apiMode,
  });

  await assertNoEnabledApplyOutsideConfirmation();

  await page.setViewportSize({ width: 390, height: 900 });
  await page.waitForTimeout(250);
  await openApplyPromotionConfirmation();
  await assertConfirmationEvidence('mobile');
  mobileScreenshotPath = await screenshot('02-mobile-omi-apply-promotion-confirmation');
...
async function writeArtifacts(exitCode) {
  ...
  const payload = {
    task: 'OMI Apply-Promotion Confirmation Browser Evidence Only',
    result,
    exitCode,
    ...
    safety: {
      ...
      applyPromotionNotRunDuringBrowserSmoke: applyPromotionRequests.length === 0,
  ...
  const reportLines = [
    ...
    '## Screenshots',
    '',
    `- Desktop: \`${path.relative(REPO_ROOT, desktopScreenshotPath || path.join(SCREENSHOT_DIR, '01-desktop-omi-apply-promotion-confirmation.png'))}\``,
    `- Mobile: \`${path.relative(REPO_ROOT, mobileScreenshotPath || path.join(SCREENSHOT_DIR, '02-mobile-omi-apply-promotion-confirmation.png'))}\``,
    '',
    '## Assertions',
    '',
    ...assertionLines,
